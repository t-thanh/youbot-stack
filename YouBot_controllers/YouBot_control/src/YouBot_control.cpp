/**********************************************************
 * This file is generated by 20-sim C++ Code Generator
 *
 *  file:  src\YouBot_control.cpp
 *  subm:  YouBot_control
 *  model: RCCMotionStack
 *  expmt: RCCMotionStack
 *  date:  November 16, 2011
 *  time:  11:40:34 am
 *  user:  Campuslicentie
 *  from:  Universiteit Twente
 *  build: 4.1.2.2
 **********************************************************/

/* Standard include files */
#include <stdio.h>
#include <math.h>

/* 20-sim include files */
#include "YouBot_control.h"

/* Orocos include */
#include <boost/algorithm/string.hpp>
#include <ocl/Component.hpp>
#include <rtt/types/SequenceTypeInfo.hpp>
#include <rtt/types/carray.hpp>
#include <ros/package.h>

using namespace Orocos;
using namespace RTT;
using namespace std;

namespace RCCMotionStack
{


	YouBot_control::YouBot_control(string name): TaskContext(name, PreOperational),TSim_config_xml("config/YouBot_control_base_config.xml")
	{
		using namespace boost;

		RTT::types::Types()->addType(new
				RTT::types::CArrayTypeInfo<RTT::types::carray<double> >("double[]"));

		initializeComputation();

		std::string path = ros::package::getPath("YouBot_control");
		TSim_config_xml=path+"/config/YouBot_control_base_config.xml";

		this->addProperty("integration_step_size", step_size ).doc("Integration step size.");
		this->addProperty("configuration_file",TSim_config_xml).doc("Path to configuation xml, relative to run directory");
		// move to configureHook
		setPeriod(step_size);
		setupParametersAndStates();

	}

	YouBot_control::~YouBot_control(void)
	{
		cleanupPropertyBags(this->properties());

		/* free memory */
		delete[] C;
		delete[] P;
		delete[] I;
		delete[] V;
		delete[] s;
		delete[] R;
		delete[] M;
		delete[] U;
		delete[] workarray;
	}

	bool YouBot_control::configureHook()
	{
		if(! TaskContext::configureHook())
		{
			return false;
		}

		return true;
	}

	/* the initialization function for submodel */
	bool YouBot_control::startHook()
	{
		if(! TaskContext::startHook())
		{
			return false;
		}

		for (vector<Adapter20Sim<RTT::InputPort<flat_matrix_t> > >::iterator it = inputPorts.begin(); it != inputPorts.end(); ++it)
		{
			flat_matrix_t temp;
			if(! it->getPort()->connected() )
			{
				log(Warning) << "InputPort (" << it->getFullName() << ") not connected." << endlog();
				//				return false;
			}
		}

		/* calculate initial and static equations */
		CalculateInitial ();
		CalculateStatic ();
		CopyInputsToVariables ();
		CalculateInput ();
		CalculateDynamic();
		CalculateOutput ();
		CopyVariablesToOutputs ();
		return true;
	}

	/* the function that calculates the submodel */
	void YouBot_control::updateHook ()
	{
		TaskContext::updateHook();

		/* another precessor submodel could determine the parameters of this submodel
		 and therefore the static parameter calculations need to be performed. */
		CalculateStatic ();

		/* main calculation of the model */
		CopyInputsToVariables (); //get input from port
		CalculateInput ();
		myintegmethod.Step();
		CalculateOutput ();
		CopyVariablesToOutputs (); //send output to port
	}

	/* the termination function for submodel */
	void YouBot_control::stopHook()
	{
		TaskContext::stopHook();

		/* copy the inputs */
		CopyInputsToVariables (); //get inputs from port

		/* calculate the final model equations */
		CalculateFinal ();

		/* set the outputs */
		CopyVariablesToOutputs (); //send output to port
	}

	/* this PRIVATE function sets the input variables from the input vector */
	//@todo Improve for multiple component inputs to have a synchronized execution.
	void YouBot_control::CopyInputsToVariables ()
	{
		/* OROCOS Entry to copy port to input array */
		for (vector<Adapter20Sim<RTT::InputPort<flat_matrix_t> > >::iterator it =
				inputPorts.begin(); it != inputPorts.end(); ++it)
		{
			flat_matrix_t temp;
			if(it->getPort()->read(temp)!=RTT::NoData)
			{
				log(Debug)<<"CopyInputsToVariables"<<endlog();
				it->setValue(temp);
			}
		}

	}

	/* this PRIVATE function uses the output variables to fill the output vector */
	void YouBot_control::CopyVariablesToOutputs ()
	{

		/* OROCOS Entry to copy output to port */
		for (vector<Adapter20Sim<RTT::OutputPort<flat_matrix_t> > >::iterator it =
				outputPorts.begin(); it != outputPorts.end(); ++it)
		{
			//	log(Debug)<<"writing out ="<<it->getValue().address()[1]<<endlog();

			//temp=it->getValue();

			//log(Debug)<<temp.address()[0]<<temp.address()[1]<<temp.address()[2]<<endlog();
			//log(Debug)<<it->getPort().getName()<<endlog();
			flat_matrix_t temp;
			temp=it->getValue();
			it->getPort()->write(temp);
		}
	}

	/* This function calculates the initial equations of the model.
	 * These equations are calculated before anything else
	 */
	void YouBot_control::CalculateInitial (void)
	{
				/* CalculateJ\initialq = [0; 0; 0; CalculateJ\InitialJointPos[1]; CalculateJ\InitialJointPos[2]; CalculateJ\InitialJointPos[3]; CalculateJ\InitialJointPos[4]; CalculateJ\InitialJointPos[5]]; */
		M[47].mat[0] = 0.0;
		M[47].mat[1] = 0.0;
		M[47].mat[2] = 0.0;
		M[47].mat[3] = M[2].mat[0];
		M[47].mat[4] = M[2].mat[1];
		M[47].mat[5] = M[2].mat[2];
		M[47].mat[6] = M[2].mat[3];
		M[47].mat[7] = M[2].mat[4];

		/* CartesianSpaceStiffness\dummy1 = CartesianSpaceStiffness\stiffness[1:3,1:3]; */
		M[59].mat[0] = M[54].mat[0];
		M[59].mat[1] = M[54].mat[1];
		M[59].mat[2] = M[54].mat[2];
		M[59].mat[3] = M[54].mat[6];
		M[59].mat[4] = M[54].mat[7];
		M[59].mat[5] = M[54].mat[8];
		M[59].mat[6] = M[54].mat[12];
		M[59].mat[7] = M[54].mat[13];
		M[59].mat[8] = M[54].mat[14];

		/* CartesianSpaceStiffness\dummy2 = CartesianSpaceStiffness\stiffness[1:3,4:6]; */
		M[60].mat[0] = M[54].mat[3];
		M[60].mat[1] = M[54].mat[4];
		M[60].mat[2] = M[54].mat[5];
		M[60].mat[3] = M[54].mat[9];
		M[60].mat[4] = M[54].mat[10];
		M[60].mat[5] = M[54].mat[11];
		M[60].mat[6] = M[54].mat[15];
		M[60].mat[7] = M[54].mat[16];
		M[60].mat[8] = M[54].mat[17];

		/* CartesianSpaceStiffness\dummy3 = CartesianSpaceStiffness\stiffness[4:6,4:6]; */
		M[61].mat[0] = M[54].mat[21];
		M[61].mat[1] = M[54].mat[22];
		M[61].mat[2] = M[54].mat[23];
		M[61].mat[3] = M[54].mat[27];
		M[61].mat[4] = M[54].mat[28];
		M[61].mat[5] = M[54].mat[29];
		M[61].mat[6] = M[54].mat[33];
		M[61].mat[7] = M[54].mat[34];
		M[61].mat[8] = M[54].mat[35];

		/* CartesianSpaceStiffness\costiffness1 = (0.5 * trace (CartesianSpaceStiffness\dummy1)) * eye (3) - CartesianSpaceStiffness\dummy1; */
		XXMatrixEye (&M[281]);
		XXScalarMatrixMul (&M[280], 0.5 * XXMatrixTrace (&M[59]), &M[281]);
		XXMatrixSub (&M[56], &M[280], &M[59]);

		/* CartesianSpaceStiffness\costiffness2 = (0.5 * trace (CartesianSpaceStiffness\dummy2)) * eye (3) - CartesianSpaceStiffness\dummy2; */
		XXMatrixEye (&M[283]);
		XXScalarMatrixMul (&M[282], 0.5 * XXMatrixTrace (&M[60]), &M[283]);
		XXMatrixSub (&M[57], &M[282], &M[60]);

		/* CartesianSpaceStiffness\costiffness3 = (0.5 * trace (CartesianSpaceStiffness\dummy3)) * eye (3) - CartesianSpaceStiffness\dummy3; */
		XXMatrixEye (&M[285]);
		XXScalarMatrixMul (&M[284], 0.5 * XXMatrixTrace (&M[61]), &M[285]);
		XXMatrixSub (&M[58], &M[284], &M[61]);

		/* GravityCompensationModel\Base\AdHik1\COMdim = GravityCompensationModel\Base\COMdim; */
		XXMatrixMov (&M[80], &M[90]);

		/* GravityCompensationModel\Base\Hij\dim = GravityCompensationModel\Base\dim; */
		XXMatrixMov (&M[88], &M[91]);

		/* GravityCompensationModel\Base\Gravity\m = GravityCompensationModel\Base\m; */
		P[146] = P[150];

		/* GravityCompensationModel\Joint12\Integrate\init = GravityCompensationModel\Joint12\InitialPos; */
		P[157] = P[158];

		/* GravityCompensationModel\Joint23\Integrate\init = GravityCompensationModel\Joint23\InitialPos; */
		P[159] = P[160];

		/* GravityCompensationModel\Joint34\Integrate\init = GravityCompensationModel\Joint34\InitialPos; */
		P[161] = P[162];

		/* GravityCompensationModel\Joint45\Integrate\init = GravityCompensationModel\Joint45\InitialPos; */
		P[163] = P[164];

		/* GravityCompensationModel\JointBase1\Integrate\init = GravityCompensationModel\JointBase1\InitialPos; */
		P[165] = P[166];

		/* GravityCompensationModel\Link1\AdHik1\COMdim = GravityCompensationModel\Link1\COMdim; */
		XXMatrixMov (&M[140], &M[150]);

		/* GravityCompensationModel\Link1\Hij\dim = GravityCompensationModel\Link1\dim; */
		XXMatrixMov (&M[148], &M[151]);

		/* GravityCompensationModel\Link1\Gravity\m = GravityCompensationModel\Link1\m; */
		P[170] = P[174];

		/* GravityCompensationModel\Link2\AdHik1\COMdim = GravityCompensationModel\Link2\COMdim; */
		XXMatrixMov (&M[159], &M[169]);

		/* GravityCompensationModel\Link2\Hij\dim = GravityCompensationModel\Link2\dim; */
		XXMatrixMov (&M[167], &M[170]);

		/* GravityCompensationModel\Link2\Gravity\m = GravityCompensationModel\Link2\m; */
		P[184] = P[188];

		/* GravityCompensationModel\Link3\AdHik1\COMdim = GravityCompensationModel\Link3\COMdim; */
		XXMatrixMov (&M[178], &M[188]);

		/* GravityCompensationModel\Link3\Hij\dim = GravityCompensationModel\Link3\dim; */
		XXMatrixMov (&M[186], &M[189]);

		/* GravityCompensationModel\Link3\Gravity\m = GravityCompensationModel\Link3\m; */
		P[198] = P[202];

		/* GravityCompensationModel\Link4\AdHik1\COMdim = GravityCompensationModel\Link4\COMdim; */
		XXMatrixMov (&M[197], &M[207]);

		/* GravityCompensationModel\Link4\Hij\dim = GravityCompensationModel\Link4\dim; */
		XXMatrixMov (&M[205], &M[208]);

		/* GravityCompensationModel\Link4\Gravity\m = GravityCompensationModel\Link4\m; */
		P[212] = P[216];

		/* GravityCompensationModel\Link5\AdHik1\COMdim = GravityCompensationModel\Link5\COMdim; */
		XXMatrixMov (&M[216], &M[226]);

		/* GravityCompensationModel\Link5\Hij\dim = GravityCompensationModel\Link5\dim; */
		XXMatrixMov (&M[224], &M[227]);

		/* GravityCompensationModel\Link5\Gravity\m = GravityCompensationModel\Link5\m; */
		P[226] = P[230];

		/* GravityCompensationModel\Base\m = GravityCompensationModel\Linkm[1]; */
		P[150] = M[235].mat[0];

		/* GravityCompensationModel\Base\COMdim = GravityCompensationModel\LinkCOMdim[1:3,1]; */
		M[90].mat[0] = M[233].mat[0];
		M[90].mat[1] = M[233].mat[6];
		M[90].mat[2] = M[233].mat[12];

		/* GravityCompensationModel\Base\dim = GravityCompensationModel\Linkdim[1:3,1]; */
		M[91].mat[0] = M[234].mat[0];
		M[91].mat[1] = M[234].mat[6];
		M[91].mat[2] = M[234].mat[12];

		/* GravityCompensationModel\Link1\m = GravityCompensationModel\Linkm[2]; */
		P[174] = M[235].mat[1];

		/* GravityCompensationModel\Link1\COMdim = GravityCompensationModel\LinkCOMdim[1:3,2]; */
		M[150].mat[0] = M[233].mat[1];
		M[150].mat[1] = M[233].mat[7];
		M[150].mat[2] = M[233].mat[13];

		/* GravityCompensationModel\Link1\dim = GravityCompensationModel\Linkdim[1:3,2]; */
		M[151].mat[0] = M[234].mat[1];
		M[151].mat[1] = M[234].mat[7];
		M[151].mat[2] = M[234].mat[13];

		/* GravityCompensationModel\Link2\m = GravityCompensationModel\Linkm[3]; */
		P[188] = M[235].mat[2];

		/* GravityCompensationModel\Link2\COMdim = GravityCompensationModel\LinkCOMdim[1:3,3]; */
		M[169].mat[0] = M[233].mat[2];
		M[169].mat[1] = M[233].mat[8];
		M[169].mat[2] = M[233].mat[14];

		/* GravityCompensationModel\Link2\dim = GravityCompensationModel\Linkdim[1:3,3]; */
		M[170].mat[0] = M[234].mat[2];
		M[170].mat[1] = M[234].mat[8];
		M[170].mat[2] = M[234].mat[14];

		/* GravityCompensationModel\Link3\m = GravityCompensationModel\Linkm[4]; */
		P[202] = M[235].mat[3];

		/* GravityCompensationModel\Link3\COMdim = GravityCompensationModel\LinkCOMdim[1:3,4]; */
		M[188].mat[0] = M[233].mat[3];
		M[188].mat[1] = M[233].mat[9];
		M[188].mat[2] = M[233].mat[15];

		/* GravityCompensationModel\Link3\dim = GravityCompensationModel\Linkdim[1:3,4]; */
		M[189].mat[0] = M[234].mat[3];
		M[189].mat[1] = M[234].mat[9];
		M[189].mat[2] = M[234].mat[15];

		/* GravityCompensationModel\Link4\m = GravityCompensationModel\Linkm[5]; */
		P[216] = M[235].mat[4];

		/* GravityCompensationModel\Link4\COMdim = GravityCompensationModel\LinkCOMdim[1:3,5]; */
		M[207].mat[0] = M[233].mat[4];
		M[207].mat[1] = M[233].mat[10];
		M[207].mat[2] = M[233].mat[16];

		/* GravityCompensationModel\Link4\dim = GravityCompensationModel\Linkdim[1:3,5]; */
		M[208].mat[0] = M[234].mat[4];
		M[208].mat[1] = M[234].mat[10];
		M[208].mat[2] = M[234].mat[16];

		/* GravityCompensationModel\Link5\m = GravityCompensationModel\Linkm[6]; */
		P[230] = M[235].mat[5];

		/* GravityCompensationModel\Link5\COMdim = GravityCompensationModel\LinkCOMdim[1:3,6]; */
		M[226].mat[0] = M[233].mat[5];
		M[226].mat[1] = M[233].mat[11];
		M[226].mat[2] = M[233].mat[17];

		/* GravityCompensationModel\Link5\dim = GravityCompensationModel\Linkdim[1:3,6]; */
		M[227].mat[0] = M[234].mat[5];
		M[227].mat[1] = M[234].mat[11];
		M[227].mat[2] = M[234].mat[17];

		/* GravityCompensationModel\JointBase1\InitialPos = GravityCompensationModel\InitialJointPos[1]; */
		P[166] = M[232].mat[0];

		/* GravityCompensationModel\Joint12\InitialPos = GravityCompensationModel\InitialJointPos[2]; */
		P[158] = M[232].mat[1];

		/* GravityCompensationModel\Joint23\InitialPos = GravityCompensationModel\InitialJointPos[3]; */
		P[160] = M[232].mat[2];

		/* GravityCompensationModel\Joint34\InitialPos = GravityCompensationModel\InitialJointPos[4]; */
		P[162] = M[232].mat[3];

		/* GravityCompensationModel\Joint45\InitialPos = GravityCompensationModel\InitialJointPos[5]; */
		P[164] = M[232].mat[4];

	}

	/* This function calculates the static equations of the model.
	 * These equations are only dependent from parameters and constants
	 */
	void YouBot_control::CalculateStatic (void)
	{
			/* CalculateJ\base_offset_H = homogeneous (eye (3), CalculateJ\baseoffset); */
	XXMatrixEye (&M[286]);
	XXMatrixHomogeneous (&M[46], &M[286], &M[5]);

	/* CalculateJ\p1ref = CalculateJ\Linkdim[1:3,1]; */
	M[28].mat[0] = M[3].mat[0];
	M[28].mat[1] = M[3].mat[6];
	M[28].mat[2] = M[3].mat[12];

	/* CalculateJ\p21 = CalculateJ\Linkdim[1:3,2]; */
	M[29].mat[0] = M[3].mat[1];
	M[29].mat[1] = M[3].mat[7];
	M[29].mat[2] = M[3].mat[13];

	/* CalculateJ\p32 = CalculateJ\Linkdim[1:3,3]; */
	M[30].mat[0] = M[3].mat[2];
	M[30].mat[1] = M[3].mat[8];
	M[30].mat[2] = M[3].mat[14];

	/* CalculateJ\p43 = CalculateJ\Linkdim[1:3,4]; */
	M[31].mat[0] = M[3].mat[3];
	M[31].mat[1] = M[3].mat[9];
	M[31].mat[2] = M[3].mat[15];

	/* CalculateJ\p54 = CalculateJ\Linkdim[1:3,5]; */
	M[32].mat[0] = M[3].mat[4];
	M[32].mat[1] = M[3].mat[10];
	M[32].mat[2] = M[3].mat[16];

	/* CalculateJ\Rtip5 = eye (3); */
	XXMatrixEye (&M[27]);

	/* CalculateJ\ptip5 = CalculateJ\Linkdim[1:3,6]; */
	M[33].mat[0] = M[3].mat[5];
	M[33].mat[1] = M[3].mat[11];
	M[33].mat[2] = M[3].mat[17];

	/* CalculateJ\Htip5 = homogeneous (CalculateJ\Rtip5, CalculateJ\ptip5); */
	XXMatrixHomogeneous (&M[40], &M[27], &M[33]);

	/* CalculateJ\uTrzrefref[1:3] = CalculateJ\omegaz; */
	M[14].mat[0] = M[8].mat[0];
	M[14].mat[1] = M[8].mat[1];
	M[14].mat[2] = M[8].mat[2];

	/* CalculateJ\uTrzrefref[4:6] = 0; */
	M[14].mat[3] = 0.0;
	M[14].mat[4] = 0.0;
	M[14].mat[5] = 0.0;

	/* CalculateJ\uTryrefref[1:3] = 0; */
	M[15].mat[0] = 0.0;
	M[15].mat[1] = 0.0;
	M[15].mat[2] = 0.0;

	/* CalculateJ\uTryrefref[4:6] = CalculateJ\vy; */
	M[15].mat[3] = M[7].mat[0];
	M[15].mat[4] = M[7].mat[1];
	M[15].mat[5] = M[7].mat[2];

	/* CalculateJ\uTrxrefref[1:3] = 0; */
	M[16].mat[0] = 0.0;
	M[16].mat[1] = 0.0;
	M[16].mat[2] = 0.0;

	/* CalculateJ\uTrxrefref[4:6] = CalculateJ\vx; */
	M[16].mat[3] = M[6].mat[0];
	M[16].mat[4] = M[6].mat[1];
	M[16].mat[5] = M[6].mat[2];

	/* CalculateJ\uTr1refref[1:3] = CalculateJ\omega1; */
	M[17].mat[0] = M[9].mat[0];
	M[17].mat[1] = M[9].mat[1];
	M[17].mat[2] = M[9].mat[2];

	/* CalculateJ\uTr1refref[4:6] = skew (CalculateJ\Linkdim[1:3,1]) * CalculateJ\omega1; */
	M[289].mat[0] = M[3].mat[0];
	M[289].mat[1] = M[3].mat[6];
	M[289].mat[2] = M[3].mat[12];
	XXMatrixSkew (&M[288], &M[289]);
	XXMatrixMul (&M[287], &M[288], &M[9]);
	M[17].mat[3] = M[287].mat[0];
	M[17].mat[4] = M[287].mat[1];
	M[17].mat[5] = M[287].mat[2];

	/* CalculateJ\uTr211[1:3] = CalculateJ\omega2; */
	M[18].mat[0] = M[10].mat[0];
	M[18].mat[1] = M[10].mat[1];
	M[18].mat[2] = M[10].mat[2];

	/* CalculateJ\uTr211[4:6] = skew (CalculateJ\Linkdim[1:3,2]) * CalculateJ\omega2; */
	M[292].mat[0] = M[3].mat[1];
	M[292].mat[1] = M[3].mat[7];
	M[292].mat[2] = M[3].mat[13];
	XXMatrixSkew (&M[291], &M[292]);
	XXMatrixMul (&M[290], &M[291], &M[10]);
	M[18].mat[3] = M[290].mat[0];
	M[18].mat[4] = M[290].mat[1];
	M[18].mat[5] = M[290].mat[2];

	/* CalculateJ\uTr322[1:3] = CalculateJ\omega3; */
	M[19].mat[0] = M[11].mat[0];
	M[19].mat[1] = M[11].mat[1];
	M[19].mat[2] = M[11].mat[2];

	/* CalculateJ\uTr322[4:6] = skew (CalculateJ\Linkdim[1:3,3]) * CalculateJ\omega3; */
	M[295].mat[0] = M[3].mat[2];
	M[295].mat[1] = M[3].mat[8];
	M[295].mat[2] = M[3].mat[14];
	XXMatrixSkew (&M[294], &M[295]);
	XXMatrixMul (&M[293], &M[294], &M[11]);
	M[19].mat[3] = M[293].mat[0];
	M[19].mat[4] = M[293].mat[1];
	M[19].mat[5] = M[293].mat[2];

	/* CalculateJ\uTr433[1:3] = CalculateJ\omega4; */
	M[20].mat[0] = M[12].mat[0];
	M[20].mat[1] = M[12].mat[1];
	M[20].mat[2] = M[12].mat[2];

	/* CalculateJ\uTr433[4:6] = skew (CalculateJ\Linkdim[1:3,4]) * CalculateJ\omega4; */
	M[298].mat[0] = M[3].mat[3];
	M[298].mat[1] = M[3].mat[9];
	M[298].mat[2] = M[3].mat[15];
	XXMatrixSkew (&M[297], &M[298]);
	XXMatrixMul (&M[296], &M[297], &M[12]);
	M[20].mat[3] = M[296].mat[0];
	M[20].mat[4] = M[296].mat[1];
	M[20].mat[5] = M[296].mat[2];

	/* CalculateJ\uTr544[1:3] = CalculateJ\omega5; */
	M[21].mat[0] = M[13].mat[0];
	M[21].mat[1] = M[13].mat[1];
	M[21].mat[2] = M[13].mat[2];

	/* CalculateJ\uTr544[4:6] = skew (CalculateJ\Linkdim[1:3,5]) * CalculateJ\omega5; */
	M[301].mat[0] = M[3].mat[4];
	M[301].mat[1] = M[3].mat[10];
	M[301].mat[2] = M[3].mat[16];
	XXMatrixSkew (&M[300], &M[301]);
	XXMatrixMul (&M[299], &M[300], &M[13]);
	M[21].mat[3] = M[299].mat[0];
	M[21].mat[4] = M[299].mat[1];
	M[21].mat[5] = M[299].mat[2];

	/* CartesianSafetyFilter1\BWrad = (CartesianSafetyFilter1\BW * 2) * 3.1415926536; */
	V[415] = (P[42] * 2.0) * 3.1415926536;

	/* GravityCompensationModel\Base\AdHik1\d = GravityCompensationModel\Base\AdHik1\COMdim; */
	XXMatrixMov (&M[81], &M[80]);

	/* GravityCompensationModel\Base\AdHik1\R = eye (3); */
	XXMatrixEye (&M[82]);

	/* GravityCompensationModel\Base\AdHik1\H = homogeneous (GravityCompensationModel\Base\AdHik1\R, GravityCompensationModel\Base\AdHik1\d); */
	XXMatrixHomogeneous (&M[83], &M[82], &M[81]);

	/* GravityCompensationModel\Base\AdHik1\AdH = Adjoint (GravityCompensationModel\Base\AdHik1\H); */
	XXMatrixAdjoint (&M[84], &M[83]);

	/* GravityCompensationModel\Base\Gravity\effort = [0; 0; 0; 0; 0; -9.8 * GravityCompensationModel\Base\Gravity\m]; */
	M[85].mat[0] = 0.0;
	M[85].mat[1] = 0.0;
	M[85].mat[2] = 0.0;
	M[85].mat[3] = 0.0;
	M[85].mat[4] = 0.0;
	M[85].mat[5] = -9.8 * P[146];

	/* GravityCompensationModel\Base\Hij\Hab = homogeneous (eye (3), GravityCompensationModel\Base\Hij\dim); */
	XXMatrixEye (&M[302]);
	XXMatrixHomogeneous (&M[87], &M[302], &M[88]);

	/* GravityCompensationModel\Hfloor0\output = eye (4); */
	XXMatrixEye (&M[92]);

	/* GravityCompensationModel\Joint12\Integrate\p = [0; 0; 0]; */
	M[97].mat[0] = 0.0;
	M[97].mat[1] = 0.0;
	M[97].mat[2] = 0.0;

	/* GravityCompensationModel\Joint23\Integrate\p = [0; 0; 0]; */
	M[105].mat[0] = 0.0;
	M[105].mat[1] = 0.0;
	M[105].mat[2] = 0.0;

	/* GravityCompensationModel\Joint34\Integrate\p = [0; 0; 0]; */
	M[113].mat[0] = 0.0;
	M[113].mat[1] = 0.0;
	M[113].mat[2] = 0.0;

	/* GravityCompensationModel\Joint45\Integrate\p = [0; 0; 0]; */
	M[121].mat[0] = 0.0;
	M[121].mat[1] = 0.0;
	M[121].mat[2] = 0.0;

	/* GravityCompensationModel\JointBase1\Integrate\p = [0; 0; 0]; */
	M[129].mat[0] = 0.0;
	M[129].mat[1] = 0.0;
	M[129].mat[2] = 0.0;

	/* GravityCompensationModel\Link1\AdHik1\d = GravityCompensationModel\Link1\AdHik1\COMdim; */
	XXMatrixMov (&M[141], &M[140]);

	/* GravityCompensationModel\Link1\AdHik1\R = eye (3); */
	XXMatrixEye (&M[142]);

	/* GravityCompensationModel\Link1\AdHik1\H = homogeneous (GravityCompensationModel\Link1\AdHik1\R, GravityCompensationModel\Link1\AdHik1\d); */
	XXMatrixHomogeneous (&M[143], &M[142], &M[141]);

	/* GravityCompensationModel\Link1\AdHik1\AdH = Adjoint (GravityCompensationModel\Link1\AdHik1\H); */
	XXMatrixAdjoint (&M[144], &M[143]);

	/* GravityCompensationModel\Link1\Gravity\effort = [0; 0; 0; 0; 0; -9.8 * GravityCompensationModel\Link1\Gravity\m]; */
	M[145].mat[0] = 0.0;
	M[145].mat[1] = 0.0;
	M[145].mat[2] = 0.0;
	M[145].mat[3] = 0.0;
	M[145].mat[4] = 0.0;
	M[145].mat[5] = -9.8 * P[170];

	/* GravityCompensationModel\Link1\Hij\Hab = homogeneous (eye (3), GravityCompensationModel\Link1\Hij\dim); */
	XXMatrixEye (&M[303]);
	XXMatrixHomogeneous (&M[147], &M[303], &M[148]);

	/* GravityCompensationModel\Link2\AdHik1\d = GravityCompensationModel\Link2\AdHik1\COMdim; */
	XXMatrixMov (&M[160], &M[159]);

	/* GravityCompensationModel\Link2\AdHik1\R = eye (3); */
	XXMatrixEye (&M[161]);

	/* GravityCompensationModel\Link2\AdHik1\H = homogeneous (GravityCompensationModel\Link2\AdHik1\R, GravityCompensationModel\Link2\AdHik1\d); */
	XXMatrixHomogeneous (&M[162], &M[161], &M[160]);

	/* GravityCompensationModel\Link2\AdHik1\AdH = Adjoint (GravityCompensationModel\Link2\AdHik1\H); */
	XXMatrixAdjoint (&M[163], &M[162]);

	/* GravityCompensationModel\Link2\Gravity\effort = [0; 0; 0; 0; 0; -9.8 * GravityCompensationModel\Link2\Gravity\m]; */
	M[164].mat[0] = 0.0;
	M[164].mat[1] = 0.0;
	M[164].mat[2] = 0.0;
	M[164].mat[3] = 0.0;
	M[164].mat[4] = 0.0;
	M[164].mat[5] = -9.8 * P[184];

	/* GravityCompensationModel\Link2\Hij\Hab = homogeneous (eye (3), GravityCompensationModel\Link2\Hij\dim); */
	XXMatrixEye (&M[304]);
	XXMatrixHomogeneous (&M[166], &M[304], &M[167]);

	/* GravityCompensationModel\Link3\AdHik1\d = GravityCompensationModel\Link3\AdHik1\COMdim; */
	XXMatrixMov (&M[179], &M[178]);

	/* GravityCompensationModel\Link3\AdHik1\R = eye (3); */
	XXMatrixEye (&M[180]);

	/* GravityCompensationModel\Link3\AdHik1\H = homogeneous (GravityCompensationModel\Link3\AdHik1\R, GravityCompensationModel\Link3\AdHik1\d); */
	XXMatrixHomogeneous (&M[181], &M[180], &M[179]);

	/* GravityCompensationModel\Link3\AdHik1\AdH = Adjoint (GravityCompensationModel\Link3\AdHik1\H); */
	XXMatrixAdjoint (&M[182], &M[181]);

	/* GravityCompensationModel\Link3\Gravity\effort = [0; 0; 0; 0; 0; -9.8 * GravityCompensationModel\Link3\Gravity\m]; */
	M[183].mat[0] = 0.0;
	M[183].mat[1] = 0.0;
	M[183].mat[2] = 0.0;
	M[183].mat[3] = 0.0;
	M[183].mat[4] = 0.0;
	M[183].mat[5] = -9.8 * P[198];

	/* GravityCompensationModel\Link3\Hij\Hab = homogeneous (eye (3), GravityCompensationModel\Link3\Hij\dim); */
	XXMatrixEye (&M[305]);
	XXMatrixHomogeneous (&M[185], &M[305], &M[186]);

	/* GravityCompensationModel\Link4\AdHik1\d = GravityCompensationModel\Link4\AdHik1\COMdim; */
	XXMatrixMov (&M[198], &M[197]);

	/* GravityCompensationModel\Link4\AdHik1\R = eye (3); */
	XXMatrixEye (&M[199]);

	/* GravityCompensationModel\Link4\AdHik1\H = homogeneous (GravityCompensationModel\Link4\AdHik1\R, GravityCompensationModel\Link4\AdHik1\d); */
	XXMatrixHomogeneous (&M[200], &M[199], &M[198]);

	/* GravityCompensationModel\Link4\AdHik1\AdH = Adjoint (GravityCompensationModel\Link4\AdHik1\H); */
	XXMatrixAdjoint (&M[201], &M[200]);

	/* GravityCompensationModel\Link4\Gravity\effort = [0; 0; 0; 0; 0; -9.8 * GravityCompensationModel\Link4\Gravity\m]; */
	M[202].mat[0] = 0.0;
	M[202].mat[1] = 0.0;
	M[202].mat[2] = 0.0;
	M[202].mat[3] = 0.0;
	M[202].mat[4] = 0.0;
	M[202].mat[5] = -9.8 * P[212];

	/* GravityCompensationModel\Link4\Hij\Hab = homogeneous (eye (3), GravityCompensationModel\Link4\Hij\dim); */
	XXMatrixEye (&M[306]);
	XXMatrixHomogeneous (&M[204], &M[306], &M[205]);

	/* GravityCompensationModel\Link5\AdHik1\d = GravityCompensationModel\Link5\AdHik1\COMdim; */
	XXMatrixMov (&M[217], &M[216]);

	/* GravityCompensationModel\Link5\AdHik1\R = eye (3); */
	XXMatrixEye (&M[218]);

	/* GravityCompensationModel\Link5\AdHik1\H = homogeneous (GravityCompensationModel\Link5\AdHik1\R, GravityCompensationModel\Link5\AdHik1\d); */
	XXMatrixHomogeneous (&M[219], &M[218], &M[217]);

	/* GravityCompensationModel\Link5\AdHik1\AdH = Adjoint (GravityCompensationModel\Link5\AdHik1\H); */
	XXMatrixAdjoint (&M[220], &M[219]);

	/* GravityCompensationModel\Link5\Gravity\effort = [0; 0; 0; 0; 0; -9.8 * GravityCompensationModel\Link5\Gravity\m]; */
	M[221].mat[0] = 0.0;
	M[221].mat[1] = 0.0;
	M[221].mat[2] = 0.0;
	M[221].mat[3] = 0.0;
	M[221].mat[4] = 0.0;
	M[221].mat[5] = -9.8 * P[226];

	/* GravityCompensationModel\Link5\Hij\Hab = homogeneous (eye (3), GravityCompensationModel\Link5\Hij\dim); */
	XXMatrixEye (&M[307]);
	XXMatrixHomogeneous (&M[223], &M[307], &M[224]);

	/* GravityCompensationModel\Mux\port.e[1] = 0; */
	M[228].mat[0] = 0.0;

	/* GravityCompensationModel\Mux\port.e[2] = 0; */
	M[228].mat[1] = 0.0;

	/* GravityCompensationModel\Mux\port.e[3] = 0; */
	M[228].mat[2] = 0.0;

	/* GravityCompensationModel\Wtip0\p.e = GravityCompensationModel\Wtip0\effort; */
	XXMatrixScalarMov (&M[229], P[238]);

	/* GravityCompensationModel\ZeroJunction\p1.f = 0; */
	XXMatrixScalarMov (&M[230], 0.0);

	/* GravityCompensationModel\ZeroJunction1\p1.f = 0; */
	XXMatrixScalarMov (&M[231], 0.0);

	/* JointSpaceDamping\R = diag (JointSpaceDamping\r); */
	XXMatrixDiag (&M[245], &M[244]);

	/* Mux\joints_velocities[1:3] = 0; */
	M[254].mat[0] = 0.0;
	M[254].mat[1] = 0.0;
	M[254].mat[2] = 0.0;

	/* GravityCompensationModel\Link5\AdHij\p1.e = transpose (Adjoint (GravityCompensationModel\Link5\Hij\Hab)) * GravityCompensationModel\Wtip0\p.e; */
	XXMatrixAdjoint (&M[309], &M[223]);
	XXMatrixTranspose (&M[308], &M[309]);
	XXMatrixMul (&M[212], &M[308], &M[229]);

	/* GravityCompensationModel\Base\AdHij\p2.f = Adjoint (GravityCompensationModel\Base\Hij\Hab) * GravityCompensationModel\ZeroJunction\p1.f; */
	XXMatrixAdjoint (&M[310], &M[87]);
	XXMatrixMul (&M[77], &M[310], &M[230]);

	/* GravityCompensationModel\Base\AdHik1\p2.f = (1.0 / GravityCompensationModel\Base\AdHik1\AdH) * GravityCompensationModel\ZeroJunction\p1.f; */
	XXScalarMatrixDiv (&M[311], 1.0, &M[84], workarray);
	XXMatrixMul (&M[79], &M[311], &M[230]);

	/* GravityCompensationModel\Base\Hij\output = GravityCompensationModel\Hfloor0\output * GravityCompensationModel\Base\Hij\Hab; */
	XXMatrixMul (&M[86], &M[92], &M[87]);

	}

	/* This function calculates the input equations of the model.
	 * These equations are dynamic equations that must not change
	 * in calls from the integration method (like random and delay).
	 */
	void YouBot_control::CalculateInput (void)
	{
		
	}

	/* This function calculates the dynamic equations of the model.
	 * These equations are called from the integration method
	 * to calculate the new model rates (that are then integrated).
	 */
	void YouBot_control::CalculateDynamic (void)
	{
			/* additional code for CartesianSafetyFilter1\s1y; */
	R[6] = s[0];
	R[7] = s[1];
	R[8] = s[2];
	R[9] = s[3];
	R[10] = s[4];
	R[11] = s[5];

	/* Mux\measured_odometry = Measured_odometry; */
	XXMatrixMov (&M[252], &M[266]);

	/* Mux\Arm1_joint_velocites = Arm1_joint_velocites; */
	XXMatrixMov (&M[253], &M[267]);

	/* CartesianSafetyFilter1\u = xyzypr; */
	XXMatrixMov (&M[49], &M[269]);

	/* PlusMinus2\plus1 = JointPoistionSetpoint; */
	XXMatrixMov (&M[261], &M[273]);

	/* Mux\Arm1_joint_states = Arm1_joint_states; */
	XXMatrixMov (&M[251], &M[274]);

	/* CartesianSafetyFilter1\s2y = CartesianSafetyFilter1\BWrad ^ 2 * (CartesianSafetyFilter1\u - CartesianSafetyFilter1\y) - (1.4142 * CartesianSafetyFilter1\BWrad) * CartesianSafetyFilter1\s1y; */
	XXMatrixSub (&M[313], &M[49], &M[278]);
	XXScalarMatrixMul (&M[312], (V[415] * V[415]), &M[313]);
	XXScalarMatrixMul (&M[314], 1.4142 * V[415], &M[277]);
	XXMatrixSub (&M[275], &M[312], &M[314]);

	/* Gain1\Rz = [cos (CartesianSafetyFilter1\y[4]), -sin (CartesianSafetyFilter1\y[4]), 0; sin (CartesianSafetyFilter1\y[4]), cos (CartesianSafetyFilter1\y[4]), 0; 0, 0, 1]; */
	M[70].mat[0] = cos (M[278].mat[3]);
	M[70].mat[1] = -(sin (M[278].mat[3]));
	M[70].mat[2] = 0.0;
	M[70].mat[3] = sin (M[278].mat[3]);
	M[70].mat[4] = cos (M[278].mat[3]);
	M[70].mat[5] = 0.0;
	M[70].mat[6] = 0.0;
	M[70].mat[7] = 0.0;
	M[70].mat[8] = 1.0;

	/* Gain1\Ry = [cos (CartesianSafetyFilter1\y[5]), 0, sin (CartesianSafetyFilter1\y[5]); 0, 1, 0; -sin (CartesianSafetyFilter1\y[5]), 0, cos (CartesianSafetyFilter1\y[5])]; */
	M[71].mat[0] = cos (M[278].mat[4]);
	M[71].mat[1] = 0.0;
	M[71].mat[2] = sin (M[278].mat[4]);
	M[71].mat[3] = 0.0;
	M[71].mat[4] = 1.0;
	M[71].mat[5] = 0.0;
	M[71].mat[6] = -(sin (M[278].mat[4]));
	M[71].mat[7] = 0.0;
	M[71].mat[8] = cos (M[278].mat[4]);

	/* Gain1\Rx = [1, 0, 0; 0, cos (CartesianSafetyFilter1\y[6]), -sin (CartesianSafetyFilter1\y[6]); 0, sin (CartesianSafetyFilter1\y[6]), cos (CartesianSafetyFilter1\y[6])]; */
	M[72].mat[0] = 1.0;
	M[72].mat[1] = 0.0;
	M[72].mat[2] = 0.0;
	M[72].mat[3] = 0.0;
	M[72].mat[4] = cos (M[278].mat[5]);
	M[72].mat[5] = -(sin (M[278].mat[5]));
	M[72].mat[6] = 0.0;
	M[72].mat[7] = sin (M[278].mat[5]);
	M[72].mat[8] = cos (M[278].mat[5]);

	/* Gain1\H = homogeneous ((Gain1\Rz * Gain1\Ry) * Gain1\Rx, CartesianSafetyFilter1\y[1:3]); */
	XXMatrixMul (&M[316], &M[70], &M[71]);
	XXMatrixMul (&M[315], &M[316], &M[72]);
	M[317].mat[0] = M[278].mat[0];
	M[317].mat[1] = M[278].mat[1];
	M[317].mat[2] = M[278].mat[2];
	XXMatrixHomogeneous (&M[69], &M[315], &M[317]);

	/* Mux\joints_velocities[4:8] = Mux\Arm1_joint_velocites; */
	M[254].mat[3] = M[253].mat[0];
	M[254].mat[4] = M[253].mat[1];
	M[254].mat[5] = M[253].mat[2];
	M[254].mat[6] = M[253].mat[3];
	M[254].mat[7] = M[253].mat[4];

	/* Mux\joint_states[1:3] = Mux\measured_odometry; */
	M[250].mat[0] = M[252].mat[0];
	M[250].mat[1] = M[252].mat[1];
	M[250].mat[2] = M[252].mat[2];

	/* Mux\joint_states[4:8] = Mux\Arm1_joint_states + Mux\joints_initial_states; */
	XXMatrixAdd (&M[318], &M[251], &M[258]);
	M[250].mat[3] = M[318].mat[0];
	M[250].mat[4] = M[318].mat[1];
	M[250].mat[5] = M[318].mat[2];
	M[250].mat[6] = M[318].mat[3];
	M[250].mat[7] = M[318].mat[4];

	/* Deadzone\output[1] = (if (Mux\joints_velocities[1] - Deadzone\n_deadzone[1]) < -30 then -30 else if (Mux\joints_velocities[1] - Deadzone\n_deadzone[1]) > 0 then 0 else (Mux\joints_velocities[1] - Deadzone\n_deadzone[1]) end end) + (if (Mux\joints_velocities[1] - Deadzone\p_deadzone[1]) < 0 then 0 else if (Mux\joints_velocities[1] - Deadzone\p_deadzone[1]) > 30 then 30 else (Mux\joints_velocities[1] - Deadzone\p_deadzone[1]) end end); */
	M[66].mat[0] = (((M[254].mat[0] - M[68].mat[0]) < -30.0) ? -30.0 : (((M[254].mat[0] - M[68].mat[0]) > 0.0) ? 0.0 : (M[254].mat[0] - M[68].mat[0]))) + (((M[254].mat[0] - M[67].mat[0]) < 0.0) ? 0.0 : (((M[254].mat[0] - M[67].mat[0]) > 30.0) ? 30.0 : (M[254].mat[0] - M[67].mat[0])));

	/* Deadzone\output[2] = (if (Mux\joints_velocities[2] - Deadzone\n_deadzone[2]) < -30 then -30 else if (Mux\joints_velocities[2] - Deadzone\n_deadzone[2]) > 0 then 0 else (Mux\joints_velocities[2] - Deadzone\n_deadzone[2]) end end) + (if (Mux\joints_velocities[2] - Deadzone\p_deadzone[2]) < 0 then 0 else if (Mux\joints_velocities[2] - Deadzone\p_deadzone[2]) > 30 then 30 else (Mux\joints_velocities[2] - Deadzone\p_deadzone[2]) end end); */
	M[66].mat[1] = (((M[254].mat[1] - M[68].mat[1]) < -30.0) ? -30.0 : (((M[254].mat[1] - M[68].mat[1]) > 0.0) ? 0.0 : (M[254].mat[1] - M[68].mat[1]))) + (((M[254].mat[1] - M[67].mat[1]) < 0.0) ? 0.0 : (((M[254].mat[1] - M[67].mat[1]) > 30.0) ? 30.0 : (M[254].mat[1] - M[67].mat[1])));

	/* Deadzone\output[3] = (if (Mux\joints_velocities[3] - Deadzone\n_deadzone[3]) < -30 then -30 else if (Mux\joints_velocities[3] - Deadzone\n_deadzone[3]) > 0 then 0 else (Mux\joints_velocities[3] - Deadzone\n_deadzone[3]) end end) + (if (Mux\joints_velocities[3] - Deadzone\p_deadzone[3]) < 0 then 0 else if (Mux\joints_velocities[3] - Deadzone\p_deadzone[3]) > 30 then 30 else (Mux\joints_velocities[3] - Deadzone\p_deadzone[3]) end end); */
	M[66].mat[2] = (((M[254].mat[2] - M[68].mat[2]) < -30.0) ? -30.0 : (((M[254].mat[2] - M[68].mat[2]) > 0.0) ? 0.0 : (M[254].mat[2] - M[68].mat[2]))) + (((M[254].mat[2] - M[67].mat[2]) < 0.0) ? 0.0 : (((M[254].mat[2] - M[67].mat[2]) > 30.0) ? 30.0 : (M[254].mat[2] - M[67].mat[2])));

	/* Deadzone\output[4] = (if (Mux\joints_velocities[4] - Deadzone\n_deadzone[4]) < -30 then -30 else if (Mux\joints_velocities[4] - Deadzone\n_deadzone[4]) > 0 then 0 else (Mux\joints_velocities[4] - Deadzone\n_deadzone[4]) end end) + (if (Mux\joints_velocities[4] - Deadzone\p_deadzone[4]) < 0 then 0 else if (Mux\joints_velocities[4] - Deadzone\p_deadzone[4]) > 30 then 30 else (Mux\joints_velocities[4] - Deadzone\p_deadzone[4]) end end); */
	M[66].mat[3] = (((M[254].mat[3] - M[68].mat[3]) < -30.0) ? -30.0 : (((M[254].mat[3] - M[68].mat[3]) > 0.0) ? 0.0 : (M[254].mat[3] - M[68].mat[3]))) + (((M[254].mat[3] - M[67].mat[3]) < 0.0) ? 0.0 : (((M[254].mat[3] - M[67].mat[3]) > 30.0) ? 30.0 : (M[254].mat[3] - M[67].mat[3])));

	/* Deadzone\output[5] = (if (Mux\joints_velocities[5] - Deadzone\n_deadzone[5]) < -30 then -30 else if (Mux\joints_velocities[5] - Deadzone\n_deadzone[5]) > 0 then 0 else (Mux\joints_velocities[5] - Deadzone\n_deadzone[5]) end end) + (if (Mux\joints_velocities[5] - Deadzone\p_deadzone[5]) < 0 then 0 else if (Mux\joints_velocities[5] - Deadzone\p_deadzone[5]) > 30 then 30 else (Mux\joints_velocities[5] - Deadzone\p_deadzone[5]) end end); */
	M[66].mat[4] = (((M[254].mat[4] - M[68].mat[4]) < -30.0) ? -30.0 : (((M[254].mat[4] - M[68].mat[4]) > 0.0) ? 0.0 : (M[254].mat[4] - M[68].mat[4]))) + (((M[254].mat[4] - M[67].mat[4]) < 0.0) ? 0.0 : (((M[254].mat[4] - M[67].mat[4]) > 30.0) ? 30.0 : (M[254].mat[4] - M[67].mat[4])));

	/* Deadzone\output[6] = (if (Mux\joints_velocities[6] - Deadzone\n_deadzone[6]) < -30 then -30 else if (Mux\joints_velocities[6] - Deadzone\n_deadzone[6]) > 0 then 0 else (Mux\joints_velocities[6] - Deadzone\n_deadzone[6]) end end) + (if (Mux\joints_velocities[6] - Deadzone\p_deadzone[6]) < 0 then 0 else if (Mux\joints_velocities[6] - Deadzone\p_deadzone[6]) > 30 then 30 else (Mux\joints_velocities[6] - Deadzone\p_deadzone[6]) end end); */
	M[66].mat[5] = (((M[254].mat[5] - M[68].mat[5]) < -30.0) ? -30.0 : (((M[254].mat[5] - M[68].mat[5]) > 0.0) ? 0.0 : (M[254].mat[5] - M[68].mat[5]))) + (((M[254].mat[5] - M[67].mat[5]) < 0.0) ? 0.0 : (((M[254].mat[5] - M[67].mat[5]) > 30.0) ? 30.0 : (M[254].mat[5] - M[67].mat[5])));

	/* Deadzone\output[7] = (if (Mux\joints_velocities[7] - Deadzone\n_deadzone[7]) < -30 then -30 else if (Mux\joints_velocities[7] - Deadzone\n_deadzone[7]) > 0 then 0 else (Mux\joints_velocities[7] - Deadzone\n_deadzone[7]) end end) + (if (Mux\joints_velocities[7] - Deadzone\p_deadzone[7]) < 0 then 0 else if (Mux\joints_velocities[7] - Deadzone\p_deadzone[7]) > 30 then 30 else (Mux\joints_velocities[7] - Deadzone\p_deadzone[7]) end end); */
	M[66].mat[6] = (((M[254].mat[6] - M[68].mat[6]) < -30.0) ? -30.0 : (((M[254].mat[6] - M[68].mat[6]) > 0.0) ? 0.0 : (M[254].mat[6] - M[68].mat[6]))) + (((M[254].mat[6] - M[67].mat[6]) < 0.0) ? 0.0 : (((M[254].mat[6] - M[67].mat[6]) > 30.0) ? 30.0 : (M[254].mat[6] - M[67].mat[6])));

	/* Deadzone\output[8] = (if (Mux\joints_velocities[8] - Deadzone\n_deadzone[8]) < -30 then -30 else if (Mux\joints_velocities[8] - Deadzone\n_deadzone[8]) > 0 then 0 else (Mux\joints_velocities[8] - Deadzone\n_deadzone[8]) end end) + (if (Mux\joints_velocities[8] - Deadzone\p_deadzone[8]) < 0 then 0 else if (Mux\joints_velocities[8] - Deadzone\p_deadzone[8]) > 30 then 30 else (Mux\joints_velocities[8] - Deadzone\p_deadzone[8]) end end); */
	M[66].mat[7] = (((M[254].mat[7] - M[68].mat[7]) < -30.0) ? -30.0 : (((M[254].mat[7] - M[68].mat[7]) > 0.0) ? 0.0 : (M[254].mat[7] - M[68].mat[7]))) + (((M[254].mat[7] - M[67].mat[7]) < 0.0) ? 0.0 : (((M[254].mat[7] - M[67].mat[7]) > 30.0) ? 30.0 : (M[254].mat[7] - M[67].mat[7])));

		/* GravityCompensationModel\Base\AdHi0\onlyRotH = GravityCompensationModel\Hfloor0\output; */
		XXMatrixMov (&M[75], &M[92]);

		/* GravityCompensationModel\Base\AdHi0\onlyRotH[1,4] = 0; */
		M[75].mat[3] = 0.0;

		/* GravityCompensationModel\Base\AdHi0\onlyRotH[2,4] = 0; */
		M[75].mat[7] = 0.0;

		/* GravityCompensationModel\Base\AdHi0\onlyRotH[3,4] = 0; */
		M[75].mat[11] = 0.0;

		/* GravityCompensationModel\Base\AdHi0\p2.e = transpose (Adjoint (GravityCompensationModel\Base\AdHi0\onlyRotH)) * GravityCompensationModel\Base\Gravity\effort; */
		XXMatrixAdjoint (&M[320], &M[75]);
		XXMatrixTranspose (&M[319], &M[320]);
		XXMatrixMul (&M[74], &M[319], &M[85]);

		/* GravityCompensationModel\Base\AdHi0\p1.f = Adjoint (GravityCompensationModel\Base\AdHi0\onlyRotH) * GravityCompensationModel\Base\AdHik1\p2.f; */
		XXMatrixAdjoint (&M[321], &M[75]);
		XXMatrixMul (&M[73], &M[321], &M[79]);

	/* GravityCompensationModel\Submodel13\port1 = Mux\joint_states[4]; */
	V[1913] = M[250].mat[3];

	/* GravityCompensationModel\Submodel13\port2 = Mux\joint_states[5]; */
	V[1914] = M[250].mat[4];

	/* GravityCompensationModel\Submodel13\port3 = Mux\joint_states[6]; */
	V[1915] = M[250].mat[5];

	/* GravityCompensationModel\Submodel13\port4 = Mux\joint_states[7]; */
	V[1916] = M[250].mat[6];

	/* GravityCompensationModel\Submodel13\port5 = Mux\joint_states[8]; */
	V[1917] = M[250].mat[7];

		/* JointSoftLimits\angle_limits_low = JointSoftLimits\working_range * JointSoftLimits\limits_low; */
		XXScalarMatrixMul (&M[241], P[310], &M[238]);

		/* JointSoftLimits\angle_limits_high = JointSoftLimits\working_range * JointSoftLimits\limits_high; */
		XXScalarMatrixMul (&M[242], P[310], &M[239]);

		/* JointSoftLimits\substate[1] = (if Mux\joint_states[1] - JointSoftLimits\angle_limits_high[1] < 0 then 0 else if Mux\joint_states[1] - JointSoftLimits\angle_limits_high[1] > 10 then 10 else Mux\joint_states[1] - JointSoftLimits\angle_limits_high[1] end end) + (if Mux\joint_states[1] - JointSoftLimits\angle_limits_low[1] < -10 then -10 else if Mux\joint_states[1] - JointSoftLimits\angle_limits_low[1] > 0 then 0 else Mux\joint_states[1] - JointSoftLimits\angle_limits_low[1] end end); */
		M[240].mat[0] = ((M[250].mat[0] - M[242].mat[0] < 0.0) ? 0.0 : ((M[250].mat[0] - M[242].mat[0] > 10.0) ? 10.0 : (M[250].mat[0] - M[242].mat[0]))) + ((M[250].mat[0] - M[241].mat[0] < -10.0) ? -10.0 : ((M[250].mat[0] - M[241].mat[0] > 0.0) ? 0.0 : (M[250].mat[0] - M[241].mat[0])));

		/* JointSoftLimits\substate[2] = (if Mux\joint_states[2] - JointSoftLimits\angle_limits_high[2] < 0 then 0 else if Mux\joint_states[2] - JointSoftLimits\angle_limits_high[2] > 10 then 10 else Mux\joint_states[2] - JointSoftLimits\angle_limits_high[2] end end) + (if Mux\joint_states[2] - JointSoftLimits\angle_limits_low[2] < -10 then -10 else if Mux\joint_states[2] - JointSoftLimits\angle_limits_low[2] > 0 then 0 else Mux\joint_states[2] - JointSoftLimits\angle_limits_low[2] end end); */
		M[240].mat[1] = ((M[250].mat[1] - M[242].mat[1] < 0.0) ? 0.0 : ((M[250].mat[1] - M[242].mat[1] > 10.0) ? 10.0 : (M[250].mat[1] - M[242].mat[1]))) + ((M[250].mat[1] - M[241].mat[1] < -10.0) ? -10.0 : ((M[250].mat[1] - M[241].mat[1] > 0.0) ? 0.0 : (M[250].mat[1] - M[241].mat[1])));

		/* JointSoftLimits\substate[3] = (if Mux\joint_states[3] - JointSoftLimits\angle_limits_high[3] < 0 then 0 else if Mux\joint_states[3] - JointSoftLimits\angle_limits_high[3] > 10 then 10 else Mux\joint_states[3] - JointSoftLimits\angle_limits_high[3] end end) + (if Mux\joint_states[3] - JointSoftLimits\angle_limits_low[3] < -10 then -10 else if Mux\joint_states[3] - JointSoftLimits\angle_limits_low[3] > 0 then 0 else Mux\joint_states[3] - JointSoftLimits\angle_limits_low[3] end end); */
		M[240].mat[2] = ((M[250].mat[2] - M[242].mat[2] < 0.0) ? 0.0 : ((M[250].mat[2] - M[242].mat[2] > 10.0) ? 10.0 : (M[250].mat[2] - M[242].mat[2]))) + ((M[250].mat[2] - M[241].mat[2] < -10.0) ? -10.0 : ((M[250].mat[2] - M[241].mat[2] > 0.0) ? 0.0 : (M[250].mat[2] - M[241].mat[2])));

		/* JointSoftLimits\substate[4] = (if Mux\joint_states[4] - JointSoftLimits\angle_limits_high[4] < 0 then 0 else if Mux\joint_states[4] - JointSoftLimits\angle_limits_high[4] > 10 then 10 else Mux\joint_states[4] - JointSoftLimits\angle_limits_high[4] end end) + (if Mux\joint_states[4] - JointSoftLimits\angle_limits_low[4] < -10 then -10 else if Mux\joint_states[4] - JointSoftLimits\angle_limits_low[4] > 0 then 0 else Mux\joint_states[4] - JointSoftLimits\angle_limits_low[4] end end); */
		M[240].mat[3] = ((M[250].mat[3] - M[242].mat[3] < 0.0) ? 0.0 : ((M[250].mat[3] - M[242].mat[3] > 10.0) ? 10.0 : (M[250].mat[3] - M[242].mat[3]))) + ((M[250].mat[3] - M[241].mat[3] < -10.0) ? -10.0 : ((M[250].mat[3] - M[241].mat[3] > 0.0) ? 0.0 : (M[250].mat[3] - M[241].mat[3])));

		/* JointSoftLimits\substate[5] = (if Mux\joint_states[5] - JointSoftLimits\angle_limits_high[5] < 0 then 0 else if Mux\joint_states[5] - JointSoftLimits\angle_limits_high[5] > 10 then 10 else Mux\joint_states[5] - JointSoftLimits\angle_limits_high[5] end end) + (if Mux\joint_states[5] - JointSoftLimits\angle_limits_low[5] < -10 then -10 else if Mux\joint_states[5] - JointSoftLimits\angle_limits_low[5] > 0 then 0 else Mux\joint_states[5] - JointSoftLimits\angle_limits_low[5] end end); */
		M[240].mat[4] = ((M[250].mat[4] - M[242].mat[4] < 0.0) ? 0.0 : ((M[250].mat[4] - M[242].mat[4] > 10.0) ? 10.0 : (M[250].mat[4] - M[242].mat[4]))) + ((M[250].mat[4] - M[241].mat[4] < -10.0) ? -10.0 : ((M[250].mat[4] - M[241].mat[4] > 0.0) ? 0.0 : (M[250].mat[4] - M[241].mat[4])));

		/* JointSoftLimits\substate[6] = (if Mux\joint_states[6] - JointSoftLimits\angle_limits_high[6] < 0 then 0 else if Mux\joint_states[6] - JointSoftLimits\angle_limits_high[6] > 10 then 10 else Mux\joint_states[6] - JointSoftLimits\angle_limits_high[6] end end) + (if Mux\joint_states[6] - JointSoftLimits\angle_limits_low[6] < -10 then -10 else if Mux\joint_states[6] - JointSoftLimits\angle_limits_low[6] > 0 then 0 else Mux\joint_states[6] - JointSoftLimits\angle_limits_low[6] end end); */
		M[240].mat[5] = ((M[250].mat[5] - M[242].mat[5] < 0.0) ? 0.0 : ((M[250].mat[5] - M[242].mat[5] > 10.0) ? 10.0 : (M[250].mat[5] - M[242].mat[5]))) + ((M[250].mat[5] - M[241].mat[5] < -10.0) ? -10.0 : ((M[250].mat[5] - M[241].mat[5] > 0.0) ? 0.0 : (M[250].mat[5] - M[241].mat[5])));

		/* JointSoftLimits\substate[7] = (if Mux\joint_states[7] - JointSoftLimits\angle_limits_high[7] < 0 then 0 else if Mux\joint_states[7] - JointSoftLimits\angle_limits_high[7] > 10 then 10 else Mux\joint_states[7] - JointSoftLimits\angle_limits_high[7] end end) + (if Mux\joint_states[7] - JointSoftLimits\angle_limits_low[7] < -10 then -10 else if Mux\joint_states[7] - JointSoftLimits\angle_limits_low[7] > 0 then 0 else Mux\joint_states[7] - JointSoftLimits\angle_limits_low[7] end end); */
		M[240].mat[6] = ((M[250].mat[6] - M[242].mat[6] < 0.0) ? 0.0 : ((M[250].mat[6] - M[242].mat[6] > 10.0) ? 10.0 : (M[250].mat[6] - M[242].mat[6]))) + ((M[250].mat[6] - M[241].mat[6] < -10.0) ? -10.0 : ((M[250].mat[6] - M[241].mat[6] > 0.0) ? 0.0 : (M[250].mat[6] - M[241].mat[6])));

		/* JointSoftLimits\substate[8] = (if Mux\joint_states[8] - JointSoftLimits\angle_limits_high[8] < 0 then 0 else if Mux\joint_states[8] - JointSoftLimits\angle_limits_high[8] > 10 then 10 else Mux\joint_states[8] - JointSoftLimits\angle_limits_high[8] end end) + (if Mux\joint_states[8] - JointSoftLimits\angle_limits_low[8] < -10 then -10 else if Mux\joint_states[8] - JointSoftLimits\angle_limits_low[8] > 0 then 0 else Mux\joint_states[8] - JointSoftLimits\angle_limits_low[8] end end); */
		M[240].mat[7] = ((M[250].mat[7] - M[242].mat[7] < 0.0) ? 0.0 : ((M[250].mat[7] - M[242].mat[7] > 10.0) ? 10.0 : (M[250].mat[7] - M[242].mat[7]))) + ((M[250].mat[7] - M[241].mat[7] < -10.0) ? -10.0 : ((M[250].mat[7] - M[241].mat[7] > 0.0) ? 0.0 : (M[250].mat[7] - M[241].mat[7])));

		/* JointSoftLimits\output = -diag (JointSoftLimits\c) * JointSoftLimits\substate; */
		XXMatrixDiag (&M[323], &M[237]);
		XXMatrixInv (&M[322], &M[323]);
		XXMatrixMul (&M[236], &M[322], &M[240]);

	/* JointSpaceDamping\output = -JointSpaceDamping\R * Deadzone\output; */
	XXMatrixInv (&M[324], &M[245]);
	XXMatrixMul (&M[243], &M[324], &M[66]);

	/* CalculateJ\Rz = [cos (Mux\joint_states[1]), -sin (Mux\joint_states[1]), 0; sin (Mux\joint_states[1]), cos (Mux\joint_states[1]), 0; 0, 0, 1]; */
	M[48].mat[0] = cos (M[250].mat[0]);
	M[48].mat[1] = -(sin (M[250].mat[0]));
	M[48].mat[2] = 0.0;
	M[48].mat[3] = sin (M[250].mat[0]);
	M[48].mat[4] = cos (M[250].mat[0]);
	M[48].mat[5] = 0.0;
	M[48].mat[6] = 0.0;
	M[48].mat[7] = 0.0;
	M[48].mat[8] = 1.0;

	/* CalculateJ\Href0 = homogeneous (CalculateJ\Rz, [Mux\joint_states[2]; Mux\joint_states[3]; 0]) * CalculateJ\base_offset_H; */
	M[326].mat[0] = M[250].mat[1];
	M[326].mat[1] = M[250].mat[2];
	M[326].mat[2] = 0.0;
	XXMatrixHomogeneous (&M[325], &M[48], &M[326]);
	XXMatrixMul (&M[34], &M[325], &M[46]);

	/* CalculateJ\R1ref = [cos (-Mux\joint_states[4]), -sin (-Mux\joint_states[4]), 0; sin (-Mux\joint_states[4]), cos (-Mux\joint_states[4]), 0; 0, 0, 1]; */
	M[22].mat[0] = cos (-M[250].mat[3]);
	M[22].mat[1] = -(sin (-M[250].mat[3]));
	M[22].mat[2] = 0.0;
	M[22].mat[3] = sin (-M[250].mat[3]);
	M[22].mat[4] = cos (-M[250].mat[3]);
	M[22].mat[5] = 0.0;
	M[22].mat[6] = 0.0;
	M[22].mat[7] = 0.0;
	M[22].mat[8] = 1.0;

	/* CalculateJ\H1ref = homogeneous (CalculateJ\R1ref, CalculateJ\p1ref); */
	XXMatrixHomogeneous (&M[35], &M[22], &M[28]);

	/* CalculateJ\R21 = [cos (Mux\joint_states[5]), 0, sin (Mux\joint_states[5]); 0, 1, 0; -sin (Mux\joint_states[5]), 0, cos (Mux\joint_states[5])]; */
	M[23].mat[0] = cos (M[250].mat[4]);
	M[23].mat[1] = 0.0;
	M[23].mat[2] = sin (M[250].mat[4]);
	M[23].mat[3] = 0.0;
	M[23].mat[4] = 1.0;
	M[23].mat[5] = 0.0;
	M[23].mat[6] = -(sin (M[250].mat[4]));
	M[23].mat[7] = 0.0;
	M[23].mat[8] = cos (M[250].mat[4]);

	/* CalculateJ\H21 = homogeneous (CalculateJ\R21, CalculateJ\p21); */
	XXMatrixHomogeneous (&M[36], &M[23], &M[29]);

	/* CalculateJ\R32 = [cos (Mux\joint_states[6]), 0, sin (Mux\joint_states[6]); 0, 1, 0; -sin (Mux\joint_states[6]), 0, cos (Mux\joint_states[6])]; */
	M[24].mat[0] = cos (M[250].mat[5]);
	M[24].mat[1] = 0.0;
	M[24].mat[2] = sin (M[250].mat[5]);
	M[24].mat[3] = 0.0;
	M[24].mat[4] = 1.0;
	M[24].mat[5] = 0.0;
	M[24].mat[6] = -(sin (M[250].mat[5]));
	M[24].mat[7] = 0.0;
	M[24].mat[8] = cos (M[250].mat[5]);

	/* CalculateJ\H32 = homogeneous (CalculateJ\R32, CalculateJ\p32); */
	XXMatrixHomogeneous (&M[37], &M[24], &M[30]);

	/* CalculateJ\R43 = [cos (Mux\joint_states[7]), 0, sin (Mux\joint_states[7]); 0, 1, 0; -sin (Mux\joint_states[7]), 0, cos (Mux\joint_states[7])]; */
	M[25].mat[0] = cos (M[250].mat[6]);
	M[25].mat[1] = 0.0;
	M[25].mat[2] = sin (M[250].mat[6]);
	M[25].mat[3] = 0.0;
	M[25].mat[4] = 1.0;
	M[25].mat[5] = 0.0;
	M[25].mat[6] = -(sin (M[250].mat[6]));
	M[25].mat[7] = 0.0;
	M[25].mat[8] = cos (M[250].mat[6]);

	/* CalculateJ\H43 = homogeneous (CalculateJ\R43, CalculateJ\p43); */
	XXMatrixHomogeneous (&M[38], &M[25], &M[31]);

	/* CalculateJ\R54 = [cos (-Mux\joint_states[8]), -sin (-Mux\joint_states[8]), 0; sin (-Mux\joint_states[8]), cos (-Mux\joint_states[8]), 0; 0, 0, 1]; */
	M[26].mat[0] = cos (-M[250].mat[7]);
	M[26].mat[1] = -(sin (-M[250].mat[7]));
	M[26].mat[2] = 0.0;
	M[26].mat[3] = sin (-M[250].mat[7]);
	M[26].mat[4] = cos (-M[250].mat[7]);
	M[26].mat[5] = 0.0;
	M[26].mat[6] = 0.0;
	M[26].mat[7] = 0.0;
	M[26].mat[8] = 1.0;

	/* CalculateJ\H54 = homogeneous (CalculateJ\R54, CalculateJ\p54); */
	XXMatrixHomogeneous (&M[39], &M[26], &M[32]);

	/* CalculateJ\H10 = CalculateJ\Href0 * CalculateJ\H1ref; */
	XXMatrixMul (&M[41], &M[34], &M[35]);

	/* CalculateJ\H20 = CalculateJ\H10 * CalculateJ\H21; */
	XXMatrixMul (&M[42], &M[41], &M[36]);

	/* CalculateJ\H30 = CalculateJ\H20 * CalculateJ\H32; */
	XXMatrixMul (&M[43], &M[42], &M[37]);

	/* CalculateJ\H40 = CalculateJ\H30 * CalculateJ\H43; */
	XXMatrixMul (&M[44], &M[43], &M[38]);

	/* CalculateJ\H50 = CalculateJ\H40 * CalculateJ\H54; */
	XXMatrixMul (&M[45], &M[44], &M[39]);

	/* CalculateJ\Htip0 = CalculateJ\H50 * CalculateJ\Htip5; */
	XXMatrixMul (&M[1], &M[45], &M[40]);

	/* CalculateJ\controllableJ[1:6,1] = Adjoint (CalculateJ\Href0) * CalculateJ\uTrzrefref; */
	XXMatrixAdjoint (&M[328], &M[34]);
	XXMatrixMul (&M[327], &M[328], &M[14]);
	M[0].mat[0] = M[327].mat[0];
	M[0].mat[8] = M[327].mat[1];
	M[0].mat[16] = M[327].mat[2];
	M[0].mat[24] = M[327].mat[3];
	M[0].mat[32] = M[327].mat[4];
	M[0].mat[40] = M[327].mat[5];

	/* CalculateJ\controllableJ[1:6,3] = Adjoint (CalculateJ\Href0) * CalculateJ\uTryrefref; */
	XXMatrixAdjoint (&M[330], &M[34]);
	XXMatrixMul (&M[329], &M[330], &M[15]);
	M[0].mat[2] = M[329].mat[0];
	M[0].mat[10] = M[329].mat[1];
	M[0].mat[18] = M[329].mat[2];
	M[0].mat[26] = M[329].mat[3];
	M[0].mat[34] = M[329].mat[4];
	M[0].mat[42] = M[329].mat[5];

	/* CalculateJ\controllableJ[1:6,2] = Adjoint (CalculateJ\Href0) * CalculateJ\uTrxrefref; */
	XXMatrixAdjoint (&M[332], &M[34]);
	XXMatrixMul (&M[331], &M[332], &M[16]);
	M[0].mat[1] = M[331].mat[0];
	M[0].mat[9] = M[331].mat[1];
	M[0].mat[17] = M[331].mat[2];
	M[0].mat[25] = M[331].mat[3];
	M[0].mat[33] = M[331].mat[4];
	M[0].mat[41] = M[331].mat[5];

	/* CalculateJ\controllableJ[1:6,4] = Adjoint (CalculateJ\Href0) * CalculateJ\uTr1refref; */
	XXMatrixAdjoint (&M[334], &M[34]);
	XXMatrixMul (&M[333], &M[334], &M[17]);
	M[0].mat[3] = M[333].mat[0];
	M[0].mat[11] = M[333].mat[1];
	M[0].mat[19] = M[333].mat[2];
	M[0].mat[27] = M[333].mat[3];
	M[0].mat[35] = M[333].mat[4];
	M[0].mat[43] = M[333].mat[5];

	/* CalculateJ\controllableJ[1:6,5] = Adjoint (CalculateJ\H10) * CalculateJ\uTr211; */
	XXMatrixAdjoint (&M[336], &M[41]);
	XXMatrixMul (&M[335], &M[336], &M[18]);
	M[0].mat[4] = M[335].mat[0];
	M[0].mat[12] = M[335].mat[1];
	M[0].mat[20] = M[335].mat[2];
	M[0].mat[28] = M[335].mat[3];
	M[0].mat[36] = M[335].mat[4];
	M[0].mat[44] = M[335].mat[5];

	/* CalculateJ\controllableJ[1:6,6] = Adjoint (CalculateJ\H20) * CalculateJ\uTr322; */
	XXMatrixAdjoint (&M[338], &M[42]);
	XXMatrixMul (&M[337], &M[338], &M[19]);
	M[0].mat[5] = M[337].mat[0];
	M[0].mat[13] = M[337].mat[1];
	M[0].mat[21] = M[337].mat[2];
	M[0].mat[29] = M[337].mat[3];
	M[0].mat[37] = M[337].mat[4];
	M[0].mat[45] = M[337].mat[5];

	/* CalculateJ\controllableJ[1:6,7] = Adjoint (CalculateJ\H30) * CalculateJ\uTr433; */
	XXMatrixAdjoint (&M[340], &M[43]);
	XXMatrixMul (&M[339], &M[340], &M[20]);
	M[0].mat[6] = M[339].mat[0];
	M[0].mat[14] = M[339].mat[1];
	M[0].mat[22] = M[339].mat[2];
	M[0].mat[30] = M[339].mat[3];
	M[0].mat[38] = M[339].mat[4];
	M[0].mat[46] = M[339].mat[5];

	/* CalculateJ\controllableJ[1:6,8] = Adjoint (CalculateJ\H40) * CalculateJ\uTr544; */
	XXMatrixAdjoint (&M[342], &M[44]);
	XXMatrixMul (&M[341], &M[342], &M[21]);
	M[0].mat[7] = M[341].mat[0];
	M[0].mat[15] = M[341].mat[1];
	M[0].mat[23] = M[341].mat[2];
	M[0].mat[31] = M[341].mat[3];
	M[0].mat[39] = M[341].mat[4];
	M[0].mat[47] = M[341].mat[5];

	/* PlusMinus2\output = PlusMinus2\plus1 - Mux\joint_states; */
	XXMatrixSub (&M[260], &M[261], &M[250]);

	/* GravityCompensationModel\Joint12\Integrate\R = [cos (GravityCompensationModel\Submodel13\port2), 0, sin (GravityCompensationModel\Submodel13\port2); 0, 1, 0; -sin (GravityCompensationModel\Submodel13\port2), 0, cos (GravityCompensationModel\Submodel13\port2)]; */
	M[96].mat[0] = cos (V[1914]);
	M[96].mat[1] = 0.0;
	M[96].mat[2] = sin (V[1914]);
	M[96].mat[3] = 0.0;
	M[96].mat[4] = 1.0;
	M[96].mat[5] = 0.0;
	M[96].mat[6] = -(sin (V[1914]));
	M[96].mat[7] = 0.0;
	M[96].mat[8] = cos (V[1914]);

	/* GravityCompensationModel\Joint12\Integrate\output = homogeneous (GravityCompensationModel\Joint12\Integrate\R, GravityCompensationModel\Joint12\Integrate\p); */
	XXMatrixHomogeneous (&M[95], &M[96], &M[97]);

	/* GravityCompensationModel\Joint23\Integrate\R = [cos (GravityCompensationModel\Submodel13\port3), 0, sin (GravityCompensationModel\Submodel13\port3); 0, 1, 0; -sin (GravityCompensationModel\Submodel13\port3), 0, cos (GravityCompensationModel\Submodel13\port3)]; */
	M[104].mat[0] = cos (V[1915]);
	M[104].mat[1] = 0.0;
	M[104].mat[2] = sin (V[1915]);
	M[104].mat[3] = 0.0;
	M[104].mat[4] = 1.0;
	M[104].mat[5] = 0.0;
	M[104].mat[6] = -(sin (V[1915]));
	M[104].mat[7] = 0.0;
	M[104].mat[8] = cos (V[1915]);

	/* GravityCompensationModel\Joint23\Integrate\output = homogeneous (GravityCompensationModel\Joint23\Integrate\R, GravityCompensationModel\Joint23\Integrate\p); */
	XXMatrixHomogeneous (&M[103], &M[104], &M[105]);

	/* GravityCompensationModel\Joint34\Integrate\R = [cos (GravityCompensationModel\Submodel13\port4), 0, sin (GravityCompensationModel\Submodel13\port4); 0, 1, 0; -sin (GravityCompensationModel\Submodel13\port4), 0, cos (GravityCompensationModel\Submodel13\port4)]; */
	M[112].mat[0] = cos (V[1916]);
	M[112].mat[1] = 0.0;
	M[112].mat[2] = sin (V[1916]);
	M[112].mat[3] = 0.0;
	M[112].mat[4] = 1.0;
	M[112].mat[5] = 0.0;
	M[112].mat[6] = -(sin (V[1916]));
	M[112].mat[7] = 0.0;
	M[112].mat[8] = cos (V[1916]);

	/* GravityCompensationModel\Joint34\Integrate\output = homogeneous (GravityCompensationModel\Joint34\Integrate\R, GravityCompensationModel\Joint34\Integrate\p); */
	XXMatrixHomogeneous (&M[111], &M[112], &M[113]);

	/* GravityCompensationModel\Joint45\Integrate\R = [cos (-GravityCompensationModel\Submodel13\port5), -sin (-GravityCompensationModel\Submodel13\port5), 0; sin (-GravityCompensationModel\Submodel13\port5), cos (-GravityCompensationModel\Submodel13\port5), 0; 0, 0, 1]; */
	M[120].mat[0] = cos (-V[1917]);
	M[120].mat[1] = -(sin (-V[1917]));
	M[120].mat[2] = 0.0;
	M[120].mat[3] = sin (-V[1917]);
	M[120].mat[4] = cos (-V[1917]);
	M[120].mat[5] = 0.0;
	M[120].mat[6] = 0.0;
	M[120].mat[7] = 0.0;
	M[120].mat[8] = 1.0;

	/* GravityCompensationModel\Joint45\Integrate\output = homogeneous (GravityCompensationModel\Joint45\Integrate\R, GravityCompensationModel\Joint45\Integrate\p); */
	XXMatrixHomogeneous (&M[119], &M[120], &M[121]);

	/* GravityCompensationModel\JointBase1\Integrate\R = [cos (-GravityCompensationModel\Submodel13\port1), -sin (-GravityCompensationModel\Submodel13\port1), 0; sin (-GravityCompensationModel\Submodel13\port1), cos (-GravityCompensationModel\Submodel13\port1), 0; 0, 0, 1]; */
	M[128].mat[0] = cos (-V[1913]);
	M[128].mat[1] = -(sin (-V[1913]));
	M[128].mat[2] = 0.0;
	M[128].mat[3] = sin (-V[1913]);
	M[128].mat[4] = cos (-V[1913]);
	M[128].mat[5] = 0.0;
	M[128].mat[6] = 0.0;
	M[128].mat[7] = 0.0;
	M[128].mat[8] = 1.0;

	/* GravityCompensationModel\JointBase1\Integrate\output = homogeneous (GravityCompensationModel\JointBase1\Integrate\R, GravityCompensationModel\JointBase1\Integrate\p); */
	XXMatrixHomogeneous (&M[127], &M[128], &M[129]);

	/* JointSpaceStiffness\output = (diag (JointSpaceStiffness\c) * (PlusMinus2\output)) * 0; */
	XXMatrixDiag (&M[344], &M[247]);
	XXMatrixMul (&M[343], &M[344], &M[260]);
	XXMatrixScalarMul (&M[246], &M[343], 0.0);

	/* Htip0 = CalculateJ\Htip0; */
	XXMatrixMov (&M[270], &M[1]);

	/* CartesianSpaceStiffness\H1_0 = inverseH (CalculateJ\Htip0) * Gain1\H; */
	XXMatrixInverseH (&M[345], &M[1]);
	XXMatrixMul (&M[55], &M[345], &M[69]);

	/* CartesianSpaceStiffness\skew21 = skew (CartesianSpaceStiffness\H1_0[1:3,4]); */
	M[346].mat[0] = M[55].mat[3];
	M[346].mat[1] = M[55].mat[7];
	M[346].mat[2] = M[55].mat[11];
	XXMatrixSkew (&M[64], &M[346]);

	/* CartesianSpaceStiffness\orientation21 = CartesianSpaceStiffness\H1_0[1:3,1:3]; */
	M[65].mat[0] = M[55].mat[0];
	M[65].mat[1] = M[55].mat[1];
	M[65].mat[2] = M[55].mat[2];
	M[65].mat[3] = M[55].mat[4];
	M[65].mat[4] = M[55].mat[5];
	M[65].mat[5] = M[55].mat[6];
	M[65].mat[6] = M[55].mat[8];
	M[65].mat[7] = M[55].mat[9];
	M[65].mat[8] = M[55].mat[10];

	/* CartesianSpaceStiffness\dummy4 = (2 * antisym (CartesianSpaceStiffness\costiffness1 * CartesianSpaceStiffness\orientation21) + antisym ((((CartesianSpaceStiffness\costiffness3 * transpose (CartesianSpaceStiffness\orientation21)) * CartesianSpaceStiffness\skew21) * CartesianSpaceStiffness\skew21) * CartesianSpaceStiffness\orientation21)) + 2 * antisym ((CartesianSpaceStiffness\costiffness2 * CartesianSpaceStiffness\skew21) * CartesianSpaceStiffness\orientation21); */
	XXMatrixMul (&M[350], &M[56], &M[65]);
	XXMatrixAsym (&M[349], &M[350], workarray);
	XXScalarMatrixMul (&M[348], 2.0, &M[349]);
	XXMatrixTranspose (&M[356], &M[65]);
	XXMatrixMul (&M[355], &M[58], &M[356]);
	XXMatrixMul (&M[354], &M[355], &M[64]);
	XXMatrixMul (&M[353], &M[354], &M[64]);
	XXMatrixMul (&M[352], &M[353], &M[65]);
	XXMatrixAsym (&M[351], &M[352], workarray);
	XXMatrixAdd (&M[347], &M[348], &M[351]);
	XXMatrixMul (&M[360], &M[57], &M[64]);
	XXMatrixMul (&M[359], &M[360], &M[65]);
	XXMatrixAsym (&M[358], &M[359], workarray);
	XXScalarMatrixMul (&M[357], 2.0, &M[358]);
	XXMatrixAdd (&M[62], &M[347], &M[357]);

	/* CartesianSpaceStiffness\dummy5 = ((transpose (CartesianSpaceStiffness\orientation21) * antisym (CartesianSpaceStiffness\costiffness3 * CartesianSpaceStiffness\skew21)) * CartesianSpaceStiffness\orientation21 + antisym (((CartesianSpaceStiffness\costiffness3 * transpose (CartesianSpaceStiffness\orientation21)) * CartesianSpaceStiffness\skew21) * CartesianSpaceStiffness\orientation21)) + 2 * antisym (CartesianSpaceStiffness\costiffness2 * CartesianSpaceStiffness\orientation21); */
	XXMatrixTranspose (&M[364], &M[65]);
	XXMatrixMul (&M[366], &M[58], &M[64]);
	XXMatrixAsym (&M[365], &M[366], workarray);
	XXMatrixMul (&M[363], &M[364], &M[365]);
	XXMatrixMul (&M[362], &M[363], &M[65]);
	XXMatrixTranspose (&M[371], &M[65]);
	XXMatrixMul (&M[370], &M[58], &M[371]);
	XXMatrixMul (&M[369], &M[370], &M[64]);
	XXMatrixMul (&M[368], &M[369], &M[65]);
	XXMatrixAsym (&M[367], &M[368], workarray);
	XXMatrixAdd (&M[361], &M[362], &M[367]);
	XXMatrixMul (&M[374], &M[57], &M[65]);
	XXMatrixAsym (&M[373], &M[374], workarray);
	XXScalarMatrixMul (&M[372], 2.0, &M[373]);
	XXMatrixAdd (&M[63], &M[361], &M[372]);

	/* CartesianSpaceStiffness\effort = Adjoint (CalculateJ\Htip0) * [CartesianSpaceStiffness\dummy4[3,2]; CartesianSpaceStiffness\dummy4[1,3]; CartesianSpaceStiffness\dummy4[2,1]; CartesianSpaceStiffness\dummy5[3,2]; CartesianSpaceStiffness\dummy5[1,3]; CartesianSpaceStiffness\dummy5[2,1]]; */
	XXMatrixAdjoint (&M[375], &M[1]);
	M[376].mat[0] = M[62].mat[7];
	M[376].mat[1] = M[62].mat[2];
	M[376].mat[2] = M[62].mat[3];
	M[376].mat[3] = M[63].mat[7];
	M[376].mat[4] = M[63].mat[2];
	M[376].mat[5] = M[63].mat[3];
	XXMatrixMul (&M[50], &M[375], &M[376]);

	/* Limit1\output[1] = (if JointSpaceStiffness\output[1] < -Limit1\force_lim then -Limit1\force_lim else if JointSpaceStiffness\output[1] > Limit1\force_lim then Limit1\force_lim else JointSpaceStiffness\output[1] end end); */
	M[248].mat[0] = (M[246].mat[0] < -P[328]) ? (-P[328]) : ((M[246].mat[0] > P[328]) ? P[328] : M[246].mat[0]);

	/* Limit1\output[2] = (if JointSpaceStiffness\output[2] < -Limit1\force_lim then -Limit1\force_lim else if JointSpaceStiffness\output[2] > Limit1\force_lim then Limit1\force_lim else JointSpaceStiffness\output[2] end end); */
	M[248].mat[1] = (M[246].mat[1] < -P[328]) ? (-P[328]) : ((M[246].mat[1] > P[328]) ? P[328] : M[246].mat[1]);

	/* Limit1\output[3] = (if JointSpaceStiffness\output[3] < -Limit1\force_lim then -Limit1\force_lim else if JointSpaceStiffness\output[3] > Limit1\force_lim then Limit1\force_lim else JointSpaceStiffness\output[3] end end); */
	M[248].mat[2] = (M[246].mat[2] < -P[328]) ? (-P[328]) : ((M[246].mat[2] > P[328]) ? P[328] : M[246].mat[2]);

	/* Limit1\output[4] = (if JointSpaceStiffness\output[4] < -Limit1\force_lim then -Limit1\force_lim else if JointSpaceStiffness\output[4] > Limit1\force_lim then Limit1\force_lim else JointSpaceStiffness\output[4] end end); */
	M[248].mat[3] = (M[246].mat[3] < -P[328]) ? (-P[328]) : ((M[246].mat[3] > P[328]) ? P[328] : M[246].mat[3]);

	/* Limit1\output[5] = (if JointSpaceStiffness\output[5] < -Limit1\force_lim then -Limit1\force_lim else if JointSpaceStiffness\output[5] > Limit1\force_lim then Limit1\force_lim else JointSpaceStiffness\output[5] end end); */
	M[248].mat[4] = (M[246].mat[4] < -P[328]) ? (-P[328]) : ((M[246].mat[4] > P[328]) ? P[328] : M[246].mat[4]);

	/* Limit1\output[6] = (if JointSpaceStiffness\output[6] < -Limit1\force_lim then -Limit1\force_lim else if JointSpaceStiffness\output[6] > Limit1\force_lim then Limit1\force_lim else JointSpaceStiffness\output[6] end end); */
	M[248].mat[5] = (M[246].mat[5] < -P[328]) ? (-P[328]) : ((M[246].mat[5] > P[328]) ? P[328] : M[246].mat[5]);

	/* Limit1\output[7] = (if JointSpaceStiffness\output[7] < -Limit1\force_lim then -Limit1\force_lim else if JointSpaceStiffness\output[7] > Limit1\force_lim then Limit1\force_lim else JointSpaceStiffness\output[7] end end); */
	M[248].mat[6] = (M[246].mat[6] < -P[328]) ? (-P[328]) : ((M[246].mat[6] > P[328]) ? P[328] : M[246].mat[6]);

	/* Limit1\output[8] = (if JointSpaceStiffness\output[8] < -Limit1\force_lim then -Limit1\force_lim else if JointSpaceStiffness\output[8] > Limit1\force_lim then Limit1\force_lim else JointSpaceStiffness\output[8] end end); */
	M[248].mat[7] = (M[246].mat[7] < -P[328]) ? (-P[328]) : ((M[246].mat[7] > P[328]) ? P[328] : M[246].mat[7]);

	/* GravityCompensationModel\JointBase1\MatrixMul\output = GravityCompensationModel\Base\Hij\output * GravityCompensationModel\JointBase1\Integrate\output; */
	XXMatrixMul (&M[130], &M[86], &M[127]);

	/* TF\joints_e = transpose (CalculateJ\controllableJ) * CartesianSpaceStiffness\effort; */
	XXMatrixTranspose (&M[377], &M[0]);
	XXMatrixMul (&M[265], &M[377], &M[50]);

	/* Limit3\output[1] = (if TF\joints_e[1] < -Limit3\force_lim then -Limit3\force_lim else if TF\joints_e[1] > Limit3\force_lim then Limit3\force_lim else TF\joints_e[1] end end); */
	M[249].mat[0] = (M[265].mat[0] < -P[329]) ? (-P[329]) : ((M[265].mat[0] > P[329]) ? P[329] : M[265].mat[0]);

	/* Limit3\output[2] = (if TF\joints_e[2] < -Limit3\force_lim then -Limit3\force_lim else if TF\joints_e[2] > Limit3\force_lim then Limit3\force_lim else TF\joints_e[2] end end); */
	M[249].mat[1] = (M[265].mat[1] < -P[329]) ? (-P[329]) : ((M[265].mat[1] > P[329]) ? P[329] : M[265].mat[1]);

	/* Limit3\output[3] = (if TF\joints_e[3] < -Limit3\force_lim then -Limit3\force_lim else if TF\joints_e[3] > Limit3\force_lim then Limit3\force_lim else TF\joints_e[3] end end); */
	M[249].mat[2] = (M[265].mat[2] < -P[329]) ? (-P[329]) : ((M[265].mat[2] > P[329]) ? P[329] : M[265].mat[2]);

	/* Limit3\output[4] = (if TF\joints_e[4] < -Limit3\force_lim then -Limit3\force_lim else if TF\joints_e[4] > Limit3\force_lim then Limit3\force_lim else TF\joints_e[4] end end); */
	M[249].mat[3] = (M[265].mat[3] < -P[329]) ? (-P[329]) : ((M[265].mat[3] > P[329]) ? P[329] : M[265].mat[3]);

	/* Limit3\output[5] = (if TF\joints_e[5] < -Limit3\force_lim then -Limit3\force_lim else if TF\joints_e[5] > Limit3\force_lim then Limit3\force_lim else TF\joints_e[5] end end); */
	M[249].mat[4] = (M[265].mat[4] < -P[329]) ? (-P[329]) : ((M[265].mat[4] > P[329]) ? P[329] : M[265].mat[4]);

	/* Limit3\output[6] = (if TF\joints_e[6] < -Limit3\force_lim then -Limit3\force_lim else if TF\joints_e[6] > Limit3\force_lim then Limit3\force_lim else TF\joints_e[6] end end); */
	M[249].mat[5] = (M[265].mat[5] < -P[329]) ? (-P[329]) : ((M[265].mat[5] > P[329]) ? P[329] : M[265].mat[5]);

	/* Limit3\output[7] = (if TF\joints_e[7] < -Limit3\force_lim then -Limit3\force_lim else if TF\joints_e[7] > Limit3\force_lim then Limit3\force_lim else TF\joints_e[7] end end); */
	M[249].mat[6] = (M[265].mat[6] < -P[329]) ? (-P[329]) : ((M[265].mat[6] > P[329]) ? P[329] : M[265].mat[6]);

	/* Limit3\output[8] = (if TF\joints_e[8] < -Limit3\force_lim then -Limit3\force_lim else if TF\joints_e[8] > Limit3\force_lim then Limit3\force_lim else TF\joints_e[8] end end); */
	M[249].mat[7] = (M[265].mat[7] < -P[329]) ? (-P[329]) : ((M[265].mat[7] > P[329]) ? P[329] : M[265].mat[7]);

	/* PlusMinus5\output = JointSpaceDamping\output + Limit3\output; */
	XXMatrixAdd (&M[264], &M[243], &M[249]);

	/* GravityCompensationModel\Link1\Hij\output = GravityCompensationModel\JointBase1\MatrixMul\output * GravityCompensationModel\Link1\Hij\Hab; */
	XXMatrixMul (&M[146], &M[130], &M[147]);

	/* PlusMinus3\output = Limit1\output + PlusMinus5\output; */
	XXMatrixAdd (&M[262], &M[248], &M[264]);

		/* GravityCompensationModel\Link1\AdHi0\onlyRotH = GravityCompensationModel\JointBase1\MatrixMul\output; */
		XXMatrixMov (&M[135], &M[130]);

		/* GravityCompensationModel\Link1\AdHi0\onlyRotH[1,4] = 0; */
		M[135].mat[3] = 0.0;

		/* GravityCompensationModel\Link1\AdHi0\onlyRotH[2,4] = 0; */
		M[135].mat[7] = 0.0;

		/* GravityCompensationModel\Link1\AdHi0\onlyRotH[3,4] = 0; */
		M[135].mat[11] = 0.0;

		/* GravityCompensationModel\Link1\AdHi0\p2.e = transpose (Adjoint (GravityCompensationModel\Link1\AdHi0\onlyRotH)) * GravityCompensationModel\Link1\Gravity\effort; */
		XXMatrixAdjoint (&M[379], &M[135]);
		XXMatrixTranspose (&M[378], &M[379]);
		XXMatrixMul (&M[134], &M[378], &M[145]);

		/* GravityCompensationModel\Link1\AdHi0\p1.f = Adjoint (GravityCompensationModel\Link1\AdHi0\onlyRotH) * GravityCompensationModel\Link1\AdHik1\p2.f; */
		XXMatrixAdjoint (&M[380], &M[135]);
		XXMatrixMul (&M[133], &M[380], &M[139]);

	/* PlusMinus4\output = JointSoftLimits\output + PlusMinus3\output; */
	XXMatrixAdd (&M[263], &M[236], &M[262]);

	/* GravityCompensationModel\Joint12\MatrixMul\output = GravityCompensationModel\Link1\Hij\output * GravityCompensationModel\Joint12\Integrate\output; */
	XXMatrixMul (&M[98], &M[146], &M[95]);

	/* GravityCompensationModel\Link1\AdHik1\p1.e = (1.0 / transpose (GravityCompensationModel\Link1\AdHik1\AdH)) * GravityCompensationModel\Link1\AdHi0\p2.e; */
	XXMatrixTranspose (&M[382], &M[144]);
	XXScalarMatrixDiv (&M[381], 1.0, &M[382], workarray);
	XXMatrixMul (&M[138], &M[381], &M[134]);

	/* GravityCompensationModel\Link2\Hij\output = GravityCompensationModel\Joint12\MatrixMul\output * GravityCompensationModel\Link2\Hij\Hab; */
	XXMatrixMul (&M[165], &M[98], &M[166]);

	/* GravityCompensationModel\Joint23\MatrixMul\output = GravityCompensationModel\Link2\Hij\output * GravityCompensationModel\Joint23\Integrate\output; */
	XXMatrixMul (&M[106], &M[165], &M[103]);

		/* GravityCompensationModel\Link2\AdHi0\onlyRotH = GravityCompensationModel\Joint12\MatrixMul\output; */
		XXMatrixMov (&M[154], &M[98]);

		/* GravityCompensationModel\Link2\AdHi0\onlyRotH[1,4] = 0; */
		M[154].mat[3] = 0.0;

		/* GravityCompensationModel\Link2\AdHi0\onlyRotH[2,4] = 0; */
		M[154].mat[7] = 0.0;

		/* GravityCompensationModel\Link2\AdHi0\onlyRotH[3,4] = 0; */
		M[154].mat[11] = 0.0;

		/* GravityCompensationModel\Link2\AdHi0\p2.e = transpose (Adjoint (GravityCompensationModel\Link2\AdHi0\onlyRotH)) * GravityCompensationModel\Link2\Gravity\effort; */
		XXMatrixAdjoint (&M[384], &M[154]);
		XXMatrixTranspose (&M[383], &M[384]);
		XXMatrixMul (&M[153], &M[383], &M[164]);

		/* GravityCompensationModel\Link2\AdHi0\p1.f = Adjoint (GravityCompensationModel\Link2\AdHi0\onlyRotH) * GravityCompensationModel\Link2\AdHik1\p2.f; */
		XXMatrixAdjoint (&M[385], &M[154]);
		XXMatrixMul (&M[152], &M[385], &M[158]);

	/* GravityCompensationModel\Link2\AdHik1\p1.e = (1.0 / transpose (GravityCompensationModel\Link2\AdHik1\AdH)) * GravityCompensationModel\Link2\AdHi0\p2.e; */
	XXMatrixTranspose (&M[387], &M[163]);
	XXScalarMatrixDiv (&M[386], 1.0, &M[387], workarray);
	XXMatrixMul (&M[157], &M[386], &M[153]);

	/* GravityCompensationModel\Link3\Hij\output = GravityCompensationModel\Joint23\MatrixMul\output * GravityCompensationModel\Link3\Hij\Hab; */
	XXMatrixMul (&M[184], &M[106], &M[185]);

	/* GravityCompensationModel\Joint34\MatrixMul\output = GravityCompensationModel\Link3\Hij\output * GravityCompensationModel\Joint34\Integrate\output; */
	XXMatrixMul (&M[114], &M[184], &M[111]);

		/* GravityCompensationModel\Link3\AdHi0\onlyRotH = GravityCompensationModel\Joint23\MatrixMul\output; */
		XXMatrixMov (&M[173], &M[106]);

		/* GravityCompensationModel\Link3\AdHi0\onlyRotH[1,4] = 0; */
		M[173].mat[3] = 0.0;

		/* GravityCompensationModel\Link3\AdHi0\onlyRotH[2,4] = 0; */
		M[173].mat[7] = 0.0;

		/* GravityCompensationModel\Link3\AdHi0\onlyRotH[3,4] = 0; */
		M[173].mat[11] = 0.0;

		/* GravityCompensationModel\Link3\AdHi0\p2.e = transpose (Adjoint (GravityCompensationModel\Link3\AdHi0\onlyRotH)) * GravityCompensationModel\Link3\Gravity\effort; */
		XXMatrixAdjoint (&M[389], &M[173]);
		XXMatrixTranspose (&M[388], &M[389]);
		XXMatrixMul (&M[172], &M[388], &M[183]);

		/* GravityCompensationModel\Link3\AdHi0\p1.f = Adjoint (GravityCompensationModel\Link3\AdHi0\onlyRotH) * GravityCompensationModel\Link3\AdHik1\p2.f; */
		XXMatrixAdjoint (&M[390], &M[173]);
		XXMatrixMul (&M[171], &M[390], &M[177]);

	/* GravityCompensationModel\Link3\AdHik1\p1.e = (1.0 / transpose (GravityCompensationModel\Link3\AdHik1\AdH)) * GravityCompensationModel\Link3\AdHi0\p2.e; */
	XXMatrixTranspose (&M[392], &M[182]);
	XXScalarMatrixDiv (&M[391], 1.0, &M[392], workarray);
	XXMatrixMul (&M[176], &M[391], &M[172]);

	/* GravityCompensationModel\Link4\Hij\output = GravityCompensationModel\Joint34\MatrixMul\output * GravityCompensationModel\Link4\Hij\Hab; */
	XXMatrixMul (&M[203], &M[114], &M[204]);

	/* GravityCompensationModel\Joint45\MatrixMul\output = GravityCompensationModel\Link4\Hij\output * GravityCompensationModel\Joint45\Integrate\output; */
	XXMatrixMul (&M[122], &M[203], &M[119]);

		/* GravityCompensationModel\Link4\AdHi0\onlyRotH = GravityCompensationModel\Joint34\MatrixMul\output; */
		XXMatrixMov (&M[192], &M[114]);

		/* GravityCompensationModel\Link4\AdHi0\onlyRotH[1,4] = 0; */
		M[192].mat[3] = 0.0;

		/* GravityCompensationModel\Link4\AdHi0\onlyRotH[2,4] = 0; */
		M[192].mat[7] = 0.0;

		/* GravityCompensationModel\Link4\AdHi0\onlyRotH[3,4] = 0; */
		M[192].mat[11] = 0.0;

		/* GravityCompensationModel\Link4\AdHi0\p2.e = transpose (Adjoint (GravityCompensationModel\Link4\AdHi0\onlyRotH)) * GravityCompensationModel\Link4\Gravity\effort; */
		XXMatrixAdjoint (&M[394], &M[192]);
		XXMatrixTranspose (&M[393], &M[394]);
		XXMatrixMul (&M[191], &M[393], &M[202]);

		/* GravityCompensationModel\Link4\AdHi0\p1.f = Adjoint (GravityCompensationModel\Link4\AdHi0\onlyRotH) * GravityCompensationModel\Link4\AdHik1\p2.f; */
		XXMatrixAdjoint (&M[395], &M[192]);
		XXMatrixMul (&M[190], &M[395], &M[196]);

	/* GravityCompensationModel\Link4\AdHik1\p1.e = (1.0 / transpose (GravityCompensationModel\Link4\AdHik1\AdH)) * GravityCompensationModel\Link4\AdHi0\p2.e; */
	XXMatrixTranspose (&M[397], &M[201]);
	XXScalarMatrixDiv (&M[396], 1.0, &M[397], workarray);
	XXMatrixMul (&M[195], &M[396], &M[191]);

		/* GravityCompensationModel\Link5\AdHi0\onlyRotH = GravityCompensationModel\Joint45\MatrixMul\output; */
		XXMatrixMov (&M[211], &M[122]);

		/* GravityCompensationModel\Link5\AdHi0\onlyRotH[1,4] = 0; */
		M[211].mat[3] = 0.0;

		/* GravityCompensationModel\Link5\AdHi0\onlyRotH[2,4] = 0; */
		M[211].mat[7] = 0.0;

		/* GravityCompensationModel\Link5\AdHi0\onlyRotH[3,4] = 0; */
		M[211].mat[11] = 0.0;

		/* GravityCompensationModel\Link5\AdHi0\p2.e = transpose (Adjoint (GravityCompensationModel\Link5\AdHi0\onlyRotH)) * GravityCompensationModel\Link5\Gravity\effort; */
		XXMatrixAdjoint (&M[399], &M[211]);
		XXMatrixTranspose (&M[398], &M[399]);
		XXMatrixMul (&M[210], &M[398], &M[221]);

		/* GravityCompensationModel\Link5\AdHi0\p1.f = Adjoint (GravityCompensationModel\Link5\AdHi0\onlyRotH) * GravityCompensationModel\Link5\AdHik1\p2.f; */
		XXMatrixAdjoint (&M[400], &M[211]);
		XXMatrixMul (&M[209], &M[400], &M[215]);

	/* GravityCompensationModel\Link5\AdHik1\p1.e = (1.0 / transpose (GravityCompensationModel\Link5\AdHik1\AdH)) * GravityCompensationModel\Link5\AdHi0\p2.e; */
	XXMatrixTranspose (&M[402], &M[220]);
	XXScalarMatrixDiv (&M[401], 1.0, &M[402], workarray);
	XXMatrixMul (&M[214], &M[401], &M[210]);

	/* GravityCompensationModel\Link5\Ta0i\p2.e = GravityCompensationModel\Link5\AdHik1\p1.e + GravityCompensationModel\Link5\AdHij\p1.e; */
	XXMatrixAdd (&M[225], &M[214], &M[212]);

	/* GravityCompensationModel\Joint45\AdHji\p1.e = (1.0 / transpose (Adjoint (GravityCompensationModel\Joint45\Integrate\output))) * GravityCompensationModel\Link5\Ta0i\p2.e; */
	XXMatrixAdjoint (&M[405], &M[119]);
	XXMatrixTranspose (&M[404], &M[405]);
	XXScalarMatrixDiv (&M[403], 1.0, &M[404], workarray);
	XXMatrixMul (&M[117], &M[403], &M[225]);

	/* GravityCompensationModel\Joint45\uTbai\p1.e = GravityCompensationModel\Joint45\AdHji\p1.e[3]; */
	V[1018] = M[117].mat[2];

	/* GravityCompensationModel\Link4\AdHij\p1.e = transpose (Adjoint (GravityCompensationModel\Link4\Hij\Hab)) * GravityCompensationModel\Joint45\AdHji\p1.e; */
	XXMatrixAdjoint (&M[407], &M[204]);
	XXMatrixTranspose (&M[406], &M[407]);
	XXMatrixMul (&M[193], &M[406], &M[117]);

	/* GravityCompensationModel\Link4\Ta0i\p2.e = GravityCompensationModel\Link4\AdHik1\p1.e + GravityCompensationModel\Link4\AdHij\p1.e; */
	XXMatrixAdd (&M[206], &M[195], &M[193]);

	/* GravityCompensationModel\Mux\port.e[8] = GravityCompensationModel\Joint45\uTbai\p1.e * GravityCompensationModel\Mux\calibartion; */
	M[228].mat[7] = V[1018] * P[237];

	/* GravityCompensationModel\Joint34\AdHji\p1.e = (1.0 / transpose (Adjoint (GravityCompensationModel\Joint34\Integrate\output))) * GravityCompensationModel\Link4\Ta0i\p2.e; */
	XXMatrixAdjoint (&M[410], &M[111]);
	XXMatrixTranspose (&M[409], &M[410]);
	XXScalarMatrixDiv (&M[408], 1.0, &M[409], workarray);
	XXMatrixMul (&M[109], &M[408], &M[206]);

	/* GravityCompensationModel\Joint34\uTbai\p1.e = GravityCompensationModel\Joint34\AdHji\p1.e[2]; */
	V[949] = M[109].mat[1];

	/* GravityCompensationModel\Link3\AdHij\p1.e = transpose (Adjoint (GravityCompensationModel\Link3\Hij\Hab)) * GravityCompensationModel\Joint34\AdHji\p1.e; */
	XXMatrixAdjoint (&M[412], &M[185]);
	XXMatrixTranspose (&M[411], &M[412]);
	XXMatrixMul (&M[174], &M[411], &M[109]);

	/* GravityCompensationModel\Link3\Ta0i\p2.e = GravityCompensationModel\Link3\AdHik1\p1.e + GravityCompensationModel\Link3\AdHij\p1.e; */
	XXMatrixAdd (&M[187], &M[176], &M[174]);

	/* GravityCompensationModel\Mux\port.e[7] = GravityCompensationModel\Joint34\uTbai\p1.e * GravityCompensationModel\Mux\calibartion; */
	M[228].mat[6] = V[949] * P[237];

	/* GravityCompensationModel\Joint23\AdHji\p1.e = (1.0 / transpose (Adjoint (GravityCompensationModel\Joint23\Integrate\output))) * GravityCompensationModel\Link3\Ta0i\p2.e; */
	XXMatrixAdjoint (&M[415], &M[103]);
	XXMatrixTranspose (&M[414], &M[415]);
	XXScalarMatrixDiv (&M[413], 1.0, &M[414], workarray);
	XXMatrixMul (&M[101], &M[413], &M[187]);

	/* GravityCompensationModel\Joint23\uTbai\p1.e = GravityCompensationModel\Joint23\AdHji\p1.e[2]; */
	V[880] = M[101].mat[1];

	/* GravityCompensationModel\Link2\AdHij\p1.e = transpose (Adjoint (GravityCompensationModel\Link2\Hij\Hab)) * GravityCompensationModel\Joint23\AdHji\p1.e; */
	XXMatrixAdjoint (&M[417], &M[166]);
	XXMatrixTranspose (&M[416], &M[417]);
	XXMatrixMul (&M[155], &M[416], &M[101]);

	/* GravityCompensationModel\Link2\Ta0i\p2.e = GravityCompensationModel\Link2\AdHik1\p1.e + GravityCompensationModel\Link2\AdHij\p1.e; */
	XXMatrixAdd (&M[168], &M[157], &M[155]);

	/* GravityCompensationModel\Mux\port.e[6] = GravityCompensationModel\Joint23\uTbai\p1.e * GravityCompensationModel\Mux\calibartion; */
	M[228].mat[5] = V[880] * P[237];

	/* GravityCompensationModel\Joint12\AdHji\p1.e = (1.0 / transpose (Adjoint (GravityCompensationModel\Joint12\Integrate\output))) * GravityCompensationModel\Link2\Ta0i\p2.e; */
	XXMatrixAdjoint (&M[420], &M[95]);
	XXMatrixTranspose (&M[419], &M[420]);
	XXScalarMatrixDiv (&M[418], 1.0, &M[419], workarray);
	XXMatrixMul (&M[93], &M[418], &M[168]);

	/* GravityCompensationModel\Joint12\uTbai\p1.e = GravityCompensationModel\Joint12\AdHji\p1.e[2]; */
	V[811] = M[93].mat[1];

	/* GravityCompensationModel\Link1\AdHij\p1.e = transpose (Adjoint (GravityCompensationModel\Link1\Hij\Hab)) * GravityCompensationModel\Joint12\AdHji\p1.e; */
	XXMatrixAdjoint (&M[422], &M[147]);
	XXMatrixTranspose (&M[421], &M[422]);
	XXMatrixMul (&M[136], &M[421], &M[93]);

	/* GravityCompensationModel\Link1\Ta0i\p2.e = GravityCompensationModel\Link1\AdHik1\p1.e + GravityCompensationModel\Link1\AdHij\p1.e; */
	XXMatrixAdd (&M[149], &M[138], &M[136]);

	/* GravityCompensationModel\Mux\port.e[5] = GravityCompensationModel\Joint12\uTbai\p1.e * GravityCompensationModel\Mux\calibartion; */
	M[228].mat[4] = V[811] * P[237];

	/* GravityCompensationModel\JointBase1\AdHji\p1.e = (1.0 / transpose (Adjoint (GravityCompensationModel\JointBase1\Integrate\output))) * GravityCompensationModel\Link1\Ta0i\p2.e; */
	XXMatrixAdjoint (&M[425], &M[127]);
	XXMatrixTranspose (&M[424], &M[425]);
	XXScalarMatrixDiv (&M[423], 1.0, &M[424], workarray);
	XXMatrixMul (&M[125], &M[423], &M[149]);

	/* GravityCompensationModel\JointBase1\uTbai\p1.e = GravityCompensationModel\JointBase1\AdHji\p1.e[3]; */
	V[1087] = M[125].mat[2];

	/* GravityCompensationModel\Mux\port.e[4] = GravityCompensationModel\JointBase1\uTbai\p1.e * GravityCompensationModel\Mux\calibartion; */
	M[228].mat[3] = V[1087] * P[237];

	/* PlusMinus1\output = PlusMinus4\output + GravityCompensationModel\Mux\port.e; */
	XXMatrixAdd (&M[259], &M[263], &M[228]);

	/* Mux\Base_twist_cmd = PlusMinus1\output[1:3]; */
	M[257].mat[0] = M[259].mat[0];
	M[257].mat[1] = M[259].mat[1];
	M[257].mat[2] = M[259].mat[2];

	/* Mux\Arm1_joint_cmd = PlusMinus1\output[4:8]; */
	M[256].mat[0] = M[259].mat[3];
	M[256].mat[1] = M[259].mat[4];
	M[256].mat[2] = M[259].mat[5];
	M[256].mat[3] = M[259].mat[6];
	M[256].mat[4] = M[259].mat[7];

	/* Arm1_joint_cmd = Mux\Arm1_joint_cmd; */
	XXMatrixMov (&M[271], &M[256]);

	/* Base_twist_cmd = Mux\Base_twist_cmd; */
	XXMatrixMov (&M[272], &M[257]);

	}

	/* This function calculates the output equations of the model.
	 * These equations are not needed for calculation of the rates
	 * and are kept separate to make the dynamic set of equations smaller.
	 * These dynamic equations are called often more than one time for each
	 * integration step that is taken. This makes model computation much faster.
	 */
	void YouBot_control::CalculateOutput (void)
	{
			/* Mux\base_joint_velocities = Base_joint_velocities; */
	XXMatrixMov (&M[255], &M[268]);

	/* GravityCompensationModel\Mux\port5.f = GravityCompensationModel\ZeroJunction1\p1.f[4]; */
	V[1908] = M[231].mat[3];

	/* GravityCompensationModel\Mux\port6.f = GravityCompensationModel\ZeroJunction1\p1.f[5]; */
	V[1909] = M[231].mat[4];

	/* GravityCompensationModel\Mux\port7.f = GravityCompensationModel\ZeroJunction1\p1.f[6]; */
	V[1910] = M[231].mat[5];

	/* GravityCompensationModel\Mux\port8.f = GravityCompensationModel\ZeroJunction1\p1.f[7]; */
	V[1911] = M[231].mat[6];

	/* GravityCompensationModel\Mux\port9.f = GravityCompensationModel\ZeroJunction1\p1.f[8]; */
	V[1912] = M[231].mat[7];

	/* GravityCompensationModel\Base\AdHik1\p1.e = (1.0 / transpose (GravityCompensationModel\Base\AdHik1\AdH)) * GravityCompensationModel\Base\AdHi0\p2.e; */
	XXMatrixTranspose (&M[427], &M[84]);
	XXScalarMatrixDiv (&M[426], 1.0, &M[427], workarray);
	XXMatrixMul (&M[78], &M[426], &M[74]);

	/* GravityCompensationModel\Joint12\uTbai\p2.f = [0; GravityCompensationModel\Mux\port6.f; 0; 0; 0; 0]; */
	M[99].mat[0] = 0.0;
	M[99].mat[1] = V[1909];
	M[99].mat[2] = 0.0;
	M[99].mat[3] = 0.0;
	M[99].mat[4] = 0.0;
	M[99].mat[5] = 0.0;

	/* GravityCompensationModel\Joint23\uTbai\p2.f = [0; GravityCompensationModel\Mux\port7.f; 0; 0; 0; 0]; */
	M[107].mat[0] = 0.0;
	M[107].mat[1] = V[1910];
	M[107].mat[2] = 0.0;
	M[107].mat[3] = 0.0;
	M[107].mat[4] = 0.0;
	M[107].mat[5] = 0.0;

	/* GravityCompensationModel\Joint34\uTbai\p2.f = [0; GravityCompensationModel\Mux\port8.f; 0; 0; 0; 0]; */
	M[115].mat[0] = 0.0;
	M[115].mat[1] = V[1911];
	M[115].mat[2] = 0.0;
	M[115].mat[3] = 0.0;
	M[115].mat[4] = 0.0;
	M[115].mat[5] = 0.0;

	/* GravityCompensationModel\Joint45\uTbai\p2.f = [0; 0; GravityCompensationModel\Mux\port9.f; 0; 0; 0]; */
	M[123].mat[0] = 0.0;
	M[123].mat[1] = 0.0;
	M[123].mat[2] = V[1912];
	M[123].mat[3] = 0.0;
	M[123].mat[4] = 0.0;
	M[123].mat[5] = 0.0;

	/* GravityCompensationModel\JointBase1\uTbai\p2.f = [0; 0; GravityCompensationModel\Mux\port5.f; 0; 0; 0]; */
	M[131].mat[0] = 0.0;
	M[131].mat[1] = 0.0;
	M[131].mat[2] = V[1908];
	M[131].mat[3] = 0.0;
	M[131].mat[4] = 0.0;
	M[131].mat[5] = 0.0;

	/* GravityCompensationModel\JointBase1\Wbai\p1.f = GravityCompensationModel\Base\AdHij\p2.f - GravityCompensationModel\JointBase1\uTbai\p2.f; */
	XXMatrixSub (&M[132], &M[77], &M[131]);

	/* GravityCompensationModel\JointBase1\AdHji\p2.f = (1.0 / Adjoint (GravityCompensationModel\JointBase1\Integrate\output)) * GravityCompensationModel\JointBase1\Wbai\p1.f; */
	XXMatrixAdjoint (&M[429], &M[127]);
	XXScalarMatrixDiv (&M[428], 1.0, &M[429], workarray);
	XXMatrixMul (&M[126], &M[428], &M[132]);

	/* GravityCompensationModel\Link1\AdHij\p2.f = Adjoint (GravityCompensationModel\Link1\Hij\Hab) * GravityCompensationModel\JointBase1\AdHji\p2.f; */
	XXMatrixAdjoint (&M[430], &M[147]);
	XXMatrixMul (&M[137], &M[430], &M[126]);

	/* GravityCompensationModel\Link1\AdHik1\p2.f = (1.0 / GravityCompensationModel\Link1\AdHik1\AdH) * GravityCompensationModel\JointBase1\AdHji\p2.f; */
	XXScalarMatrixDiv (&M[431], 1.0, &M[144], workarray);
	XXMatrixMul (&M[139], &M[431], &M[126]);

	/* GravityCompensationModel\Joint12\Wbai\p3.f = GravityCompensationModel\Link1\AdHij\p2.f - GravityCompensationModel\Joint12\uTbai\p2.f; */
	XXMatrixSub (&M[100], &M[137], &M[99]);

	/* GravityCompensationModel\Joint12\AdHji\p2.f = (1.0 / Adjoint (GravityCompensationModel\Joint12\Integrate\output)) * GravityCompensationModel\Joint12\Wbai\p3.f; */
	XXMatrixAdjoint (&M[433], &M[95]);
	XXScalarMatrixDiv (&M[432], 1.0, &M[433], workarray);
	XXMatrixMul (&M[94], &M[432], &M[100]);

	/* GravityCompensationModel\Link2\AdHij\p2.f = Adjoint (GravityCompensationModel\Link2\Hij\Hab) * GravityCompensationModel\Joint12\AdHji\p2.f; */
	XXMatrixAdjoint (&M[434], &M[166]);
	XXMatrixMul (&M[156], &M[434], &M[94]);

	/* GravityCompensationModel\Link2\AdHik1\p2.f = (1.0 / GravityCompensationModel\Link2\AdHik1\AdH) * GravityCompensationModel\Joint12\AdHji\p2.f; */
	XXScalarMatrixDiv (&M[435], 1.0, &M[163], workarray);
	XXMatrixMul (&M[158], &M[435], &M[94]);

	/* GravityCompensationModel\Joint23\Wbai\p3.f = GravityCompensationModel\Link2\AdHij\p2.f - GravityCompensationModel\Joint23\uTbai\p2.f; */
	XXMatrixSub (&M[108], &M[156], &M[107]);

	/* GravityCompensationModel\Joint23\AdHji\p2.f = (1.0 / Adjoint (GravityCompensationModel\Joint23\Integrate\output)) * GravityCompensationModel\Joint23\Wbai\p3.f; */
	XXMatrixAdjoint (&M[437], &M[103]);
	XXScalarMatrixDiv (&M[436], 1.0, &M[437], workarray);
	XXMatrixMul (&M[102], &M[436], &M[108]);

	/* GravityCompensationModel\Link3\AdHij\p2.f = Adjoint (GravityCompensationModel\Link3\Hij\Hab) * GravityCompensationModel\Joint23\AdHji\p2.f; */
	XXMatrixAdjoint (&M[438], &M[185]);
	XXMatrixMul (&M[175], &M[438], &M[102]);

	/* GravityCompensationModel\Link3\AdHik1\p2.f = (1.0 / GravityCompensationModel\Link3\AdHik1\AdH) * GravityCompensationModel\Joint23\AdHji\p2.f; */
	XXScalarMatrixDiv (&M[439], 1.0, &M[182], workarray);
	XXMatrixMul (&M[177], &M[439], &M[102]);

	/* GravityCompensationModel\Joint34\Wbai\p3.f = GravityCompensationModel\Link3\AdHij\p2.f - GravityCompensationModel\Joint34\uTbai\p2.f; */
	XXMatrixSub (&M[116], &M[175], &M[115]);

	/* GravityCompensationModel\Joint34\AdHji\p2.f = (1.0 / Adjoint (GravityCompensationModel\Joint34\Integrate\output)) * GravityCompensationModel\Joint34\Wbai\p3.f; */
	XXMatrixAdjoint (&M[441], &M[111]);
	XXScalarMatrixDiv (&M[440], 1.0, &M[441], workarray);
	XXMatrixMul (&M[110], &M[440], &M[116]);

	/* GravityCompensationModel\Link4\AdHij\p2.f = Adjoint (GravityCompensationModel\Link4\Hij\Hab) * GravityCompensationModel\Joint34\AdHji\p2.f; */
	XXMatrixAdjoint (&M[442], &M[204]);
	XXMatrixMul (&M[194], &M[442], &M[110]);

	/* GravityCompensationModel\Link4\AdHik1\p2.f = (1.0 / GravityCompensationModel\Link4\AdHik1\AdH) * GravityCompensationModel\Joint34\AdHji\p2.f; */
	XXScalarMatrixDiv (&M[443], 1.0, &M[201], workarray);
	XXMatrixMul (&M[196], &M[443], &M[110]);

	/* GravityCompensationModel\Joint45\Wbai\p1.f = GravityCompensationModel\Link4\AdHij\p2.f - GravityCompensationModel\Joint45\uTbai\p2.f; */
	XXMatrixSub (&M[124], &M[194], &M[123]);

	/* GravityCompensationModel\Link5\Hij\output = GravityCompensationModel\Joint45\MatrixMul\output * GravityCompensationModel\Link5\Hij\Hab; */
	XXMatrixMul (&M[222], &M[122], &M[223]);

	/* GravityCompensationModel\Joint45\AdHji\p2.f = (1.0 / Adjoint (GravityCompensationModel\Joint45\Integrate\output)) * GravityCompensationModel\Joint45\Wbai\p1.f; */
	XXMatrixAdjoint (&M[445], &M[119]);
	XXScalarMatrixDiv (&M[444], 1.0, &M[445], workarray);
	XXMatrixMul (&M[118], &M[444], &M[124]);

	/* GravityCompensationModel\Link5\AdHij\p2.f = Adjoint (GravityCompensationModel\Link5\Hij\Hab) * GravityCompensationModel\Joint45\AdHji\p2.f; */
	XXMatrixAdjoint (&M[446], &M[223]);
	XXMatrixMul (&M[213], &M[446], &M[118]);

	/* GravityCompensationModel\Link5\AdHik1\p2.f = (1.0 / GravityCompensationModel\Link5\AdHik1\AdH) * GravityCompensationModel\Joint45\AdHji\p2.f; */
	XXScalarMatrixDiv (&M[447], 1.0, &M[220], workarray);
	XXMatrixMul (&M[215], &M[447], &M[118]);

	/* GravityCompensationModel\Base\AdHij\p1.e = transpose (Adjoint (GravityCompensationModel\Base\Hij\Hab)) * GravityCompensationModel\JointBase1\AdHji\p1.e; */
	XXMatrixAdjoint (&M[449], &M[87]);
	XXMatrixTranspose (&M[448], &M[449]);
	XXMatrixMul (&M[76], &M[448], &M[125]);

	/* GravityCompensationModel\Base\Ta0i\p2.e = GravityCompensationModel\Base\AdHik1\p1.e + GravityCompensationModel\Base\AdHij\p1.e; */
	XXMatrixAdd (&M[89], &M[78], &M[76]);

			}

	/* This function calculates the final equations of the model.
	 * These equations are calculated after all the calculations
	 * are performed
	 */
	void YouBot_control::CalculateFinal (void)
	{
		
	}

	void YouBot_control::setupParametersAndStates()
	{
		using namespace boost;

		TiXmlDocument doc(TSim_config_xml);
		if (!doc.LoadFile())
		{
			log(Error) << "File not found: " << TSim_config_xml << endlog();
			return;
		}

		TiXmlHandle hdoc(&doc);
		TiXmlElement* pElem;
		TiXmlHandle hRoot(0);
		TiXmlNode* tNode(NULL);

		hRoot = TiXmlHandle(hdoc.FirstChildElement().Element());
		pElem = hRoot.FirstChild("modelVariables").FirstChild().Element();

		if (pElem)
		{

			do
			{
				log(Debug) << "Begin of processing the tag" << endlog();
				log(Debug) << "Read all relevant fields" << endlog();
				const char * kind =
				(tNode = pElem->FirstChild("kind")) == NULL ?
				NULL : tNode->ToElement()->GetText();
				const char * name =
				(tNode = pElem->FirstChild("name")) == NULL ?
				NULL : tNode->ToElement()->GetText();
				name =
				name == NULL || name == "" ?
				" " : name;
				const char * description =
				(tNode = pElem->FirstChild("description")) == NULL ?
				" " : tNode->ToElement()->GetText();
				description =
				description == NULL || description == "" ?
				" " : description;
				const char * container =
				(tNode = pElem->FirstChild("storage")) == NULL ?
				NULL :
				tNode->FirstChild("name")->ToElement()->GetText();
				const char * strIndex =
				(tNode = pElem->FirstChild("storage")) == NULL ?
				NULL :
				tNode->FirstChild("index")->ToElement()->GetText();
				const char * strRows =
				(tNode = pElem->FirstChild("size")) == NULL ?
				NULL :
				tNode->FirstChild("rows")->ToElement()->GetText();
				const char * strColumns =
				(tNode = pElem->FirstChild("size")) == NULL ?
				NULL :
				tNode->FirstChild("columns")->ToElement()->GetText();
				const char * type =
				(tNode = pElem->FirstChild("type")) == NULL ?
				NULL : tNode->ToElement()->GetText();

				const char * value=
				(tNode = pElem->FirstChild("value")) == NULL ?
				NULL : tNode->ToElement()->GetText();
				log(Debug) << " strings to numbers" << endlog();
				int index, rows, columns;
				if (strIndex == NULL)
				{
					log(Debug)
					<< "Processing : "
					<< TSim_config_xml
					<< " Link to 20 sim variables incorrectly defined: Index =null \n"
					<< " The node will be not processed" << endlog();
					continue;
				}
				index = atoi(strIndex);
				rows = strRows == NULL ? 1 : atoi(strRows);
				columns = strColumns == NULL ? 1 : atoi(strColumns);

				if(index == -1 || rows == -1 || columns == -1)
				{
					log(Error) << "XML parse error: ";
					log(Error) << "Name: " << name << " Disc: " << description
					<< " Index: " << index << " Kind: " << kind << " Type: "
					<< type << endlog();
				}

				log(Debug) << "Name: " << name << " Disc: " << description
				<< " Index: " << index << " Kind: " << kind << " Type: "
				<< type << endlog();

				common20sim::XVMatrix* tempXVMatrix;
				log(Debug) << " Selecting source of data for this node" << endlog();
				if (boost::equals(container, "V"))
				{
					tempXVMatrix=new common20sim::XVMatrix(V+index, rows, columns);
				}
				else if (boost::equals(container, "C"))
				{
					tempXVMatrix=new common20sim::XVMatrix(C+index, rows, columns);
				}
				else if (boost::equals(container, "P"))
				{
					tempXVMatrix=new common20sim::XVMatrix(P+index, rows, columns);
				}
				else if (boost::equals(container, "I"))
				{
					tempXVMatrix=new common20sim::XVMatrix(I+index, rows, columns);
				}
				else if (boost::equals(container, "s"))
				{
					tempXVMatrix=new common20sim::XVMatrix(s+index, rows, columns);
				}
				else if (boost::equals(container, "R"))
				{
					tempXVMatrix=new common20sim::XVMatrix(R+index, rows, columns);
				}
				else if (boost::equals(container, "M"))
				{
					tempXVMatrix=new common20sim::XVMatrix(M[index]);
				}
				else if (boost::equals(container, "U"))
				{
					tempXVMatrix=new common20sim::XVMatrix(U+index, rows, columns);
				}
				else if (boost::equals(container, "workarray"))
				{
					tempXVMatrix=new common20sim::XVMatrix(workarray+index, rows, columns);
				}
				else
				{
					log(Debug) << "Processing : " << TSim_config_xml
					<< " Link to 20 sim variables incorrectly defined: "
					<< container << " The node will be not processed"
					<< endlog();
					continue;
				}

				log(Debug)<<"load matrix with parameters from xml"<<endlog();
				log(Debug)<<"Read from xml values: \t "<<value<<endlog();
				loadMatrixValue(value,tempXVMatrix);
				log(Debug)<<"Confirm from XVmatrix: \t "<<*tempXVMatrix<<endlog();
				log(Debug)<<"select the action based o kind of parameter"<<endlog();
				log(Debug)<<"select the action based o kind of parameter"<<endlog();

				if (boost::equals(kind, "parameter"))
				{
					// create port decorator
					//RTT::Property<RTT::types::carray<double> >* p_rttPort; //TODO: Fix me!

					Adapter20Sim<RTT::Property<RTT::types::carray<double> > > p_20simport(
							string(name), string(description), tempXVMatrix, NULL);

					// check if the property should be hierarchical folded
					RTT::PropertyBag* p_bag(NULL);
					p_bag = createHierarchicalPropertyBags(name);

					p_bag->addProperty(p_20simport.getShortName(), p_20simport.getLink()->getCArray());
					// save the link for updates
					propertyPorts.push_back(p_20simport);

				}
				else if (boost::equals(kind, "state"))
				{

				}
				else if (boost::equals(kind, "variable"))
				{
					//not implemented yet :)
					log(Debug)<<"Variables are not exhibited to out side of the component"<<endlog();
					// Recommend update is to show Interesting variables
				}
				else if (boost::equals(kind, "input"))
				{
					// create port decorator
					RTT::InputPort<flat_matrix_t> * p_rttPort = new RTT::InputPort<flat_matrix_t>;

					Adapter20Sim<RTT::InputPort<flat_matrix_t> > p_20simport(
							string(name), string(description), tempXVMatrix, p_rttPort);

					// create a port
					this->addPort(p_20simport.getFullName(), *p_rttPort).doc(
							p_20simport.getDescription());
					//save link for updates

					inputPorts.push_back(p_20simport);

				}
				else if (boost::equals(kind, "output"))
				{
					// create port decorator
					RTT::OutputPort<flat_matrix_t> * p_rttPort = new RTT::OutputPort<flat_matrix_t>;

					Adapter20Sim<RTT::OutputPort<flat_matrix_t> > p_20simport(
							string(name), string(description), tempXVMatrix, p_rttPort);

					// create a port
					this->addPort(p_20simport.getFullName(), *p_rttPort).doc(
							p_20simport.getDescription());

					outputPorts.push_back(p_20simport);

				}
				else
				{
					log(Debug) << TSim_config_xml << " token kind not recognized("
					<< kind << ")" << endlog();
					continue;
				}
				log(Debug) << "End of processing the tag" << endlog();
			}while ( (pElem = pElem->NextSiblingElement()) != NULL);
			log(Debug) << "End of processing the xml" << endlog();
		}
	}
	void YouBot_control::loadMatrixValue(const char * input,XVMatrix *output)
	{

		std::size_t position=0;
		istringstream iss_input(input);

		do
		{
			std::string row;
			getline(iss_input,row,';');
			istringstream iss_row(row);

			do
			{
				std::string field;
				getline(iss_row,field,',');
				output->at(position)=atof(field.c_str());
				position++;
			}while(!iss_row.eof());

		}while(!iss_input.eof());
	}

	RTT::PropertyBag* YouBot_control::createHierarchicalPropertyBags(const char * name)
	{
		using namespace boost;
		RTT::PropertyBag* p_bag(NULL);
		string cleaned_name;

		typedef split_iterator<const char*> ssi;
		ssi end;
		for (ssi it = make_split_iterator(name, first_finder("\\", is_iequal()));
				it != end;)
		{
			cleaned_name = replaceIllegalCharacter(
					copy_range<std::string>(*it));

			// Are we at the end already?
			if (++it == end)
			{
				break; // last name of the string is the name of the property.
			}

			if (p_bag == NULL)
			{
				RTT::Property<PropertyBag>* ppb = dynamic_cast<RTT::Property<
				PropertyBag>*>(this->getProperty(cleaned_name));
				if(ppb == NULL)
				{
					p_bag = new RTT::PropertyBag;
					this->addProperty(cleaned_name, *p_bag).doc("Submodel parameters");
				}
				else
				{
					p_bag = &(ppb->value());
				}
			}
			else
			{
				RTT::Property<PropertyBag>* ppb = dynamic_cast<RTT::Property<PropertyBag>*>(p_bag->getProperty(cleaned_name));
				RTT::PropertyBag* ptmp(NULL);
				if(ppb == NULL)
				{
					ptmp = new RTT::PropertyBag;
					p_bag->addProperty(cleaned_name, *ptmp).doc("Submodel parameters");
				}
				else
				{
					ptmp = &(ppb->value());
				}
				p_bag = ptmp;
			}
		}
		if (p_bag != NULL)
		{
			return p_bag;
		}
		else
		{
			return this->properties();

		}
	}
	void YouBot_control::cleanupPropertyBags(RTT::PropertyBag* p)
	{
		RTT::PropertyBag::iterator it(p->begin());
		RTT::PropertyBag::iterator end(p->end());

		RTT::Property<PropertyBag>* ppb(NULL);

		for(;it != end; ++it)
		{
			ppb = dynamic_cast<RTT::Property<PropertyBag>*>(*it);

			if(ppb != NULL)
			{
				PropertyBag* ptmp = &(ppb->value());
				cleanupPropertyBags(ptmp);
				delete ptmp;
			}
		}
	}

	bool YouBot_control::setPeriod(RTT::Seconds s)
	{
		if(TaskContext::setPeriod(s))
		{
			step_size = s;
			return true;
		}
		else
		{
			return false;
		}
	}
	bool YouBot_control::initializeComputation()
	{
		start_time = 0.0;
		finish_time = 200.0;
		step_size = 0.001;
		time = 0;
		major = true;

		number_constants = 24;
		number_parameters = 335;
		number_initialvalues = 12;
		number_variables = 2218;
		number_states = 12;
		number_rates = 12;
		number_matrices = 450;
		number_unnamed = 4060;

		/* the variable arrays */
		C = new XXDouble[24 + 1]; /* constants */
		P = new XXDouble[335 + 1]; /* parameters, currently only one type of parameter exists: double */
		I = new XXDouble[12 + 1]; /* initial values */
		V = new XXDouble[2218 + 1]; /* variables */

		s = new XXDouble[12 + 1]; /* states */
		R = new XXDouble[12 + 1]; /* rates (or new states) */
		M = new XXMatrix[450 + 1]; /* matrices */
		U = new XXDouble[4060 + 1]; /* unnamed */
		workarray = new XXDouble[120 + 1];

		myintegmethod.Initialize(this);

		/* initialization phase (allocating memory) */
		initialize = true;

		//time = t;
			C[0] = 1.0;		/* CalculateJ\vx */
	C[1] = 0.0;		
	C[2] = 0.0;		
	C[3] = 0.0;		/* CalculateJ\vy */
	C[4] = 1.0;		
	C[5] = 0.0;		
	C[6] = 0.0;		/* CalculateJ\omegaz */
	C[7] = 0.0;		
	C[8] = 1.0;		
	C[9] = 0.0;		/* CalculateJ\omega1 */
	C[10] = 0.0;		
	C[11] = -1.0;		
	C[12] = 0.0;		/* CalculateJ\omega2 */
	C[13] = 1.0;		
	C[14] = 0.0;		
	C[15] = 0.0;		/* CalculateJ\omega3 */
	C[16] = 1.0;		
	C[17] = 0.0;		
	C[18] = 0.0;		/* CalculateJ\omega4 */
	C[19] = 1.0;		
	C[20] = 0.0;		
	C[21] = 0.0;		/* CalculateJ\omega5 */
	C[22] = 0.0;		
	C[23] = -1.0;		

		/* set the states */
			s[0] = I[0];		/* CartesianSafetyFilter1\s1y */
	s[1] = I[1];
	s[2] = I[2];
	s[3] = I[3];
	s[4] = I[4];
	s[5] = I[5];
	s[6] = I[6];		/* CartesianSafetyFilter1\y */
	s[7] = I[7];
	s[8] = I[8];
	s[9] = I[9];
	s[10] = I[10];
	s[11] = I[11];


		/* set the matrices */
			M[0].mat = &V[0];		/* CalculateJ\controllableJ */
	M[0].rows = 6;
	M[0].columns = 8;
	M[1].mat = &V[48];		/* CalculateJ\Htip0 */
	M[1].rows = 4;
	M[1].columns = 4;
	M[2].mat = &P[0];		/* CalculateJ\InitialJointPos */
	M[2].rows = 5;
	M[2].columns = 1;
	M[3].mat = &P[5];		/* CalculateJ\Linkdim */
	M[3].rows = 3;
	M[3].columns = 6;
	M[4].mat = &P[23];		/* CalculateJ\InitialBasePos */
	M[4].rows = 4;
	M[4].columns = 4;
	M[5].mat = &P[39];		/* CalculateJ\baseoffset */
	M[5].rows = 3;
	M[5].columns = 1;
	M[6].mat = &C[0];		/* CalculateJ\vx */
	M[6].rows = 3;
	M[6].columns = 1;
	M[7].mat = &C[3];		/* CalculateJ\vy */
	M[7].rows = 3;
	M[7].columns = 1;
	M[8].mat = &C[6];		/* CalculateJ\omegaz */
	M[8].rows = 3;
	M[8].columns = 1;
	M[9].mat = &C[9];		/* CalculateJ\omega1 */
	M[9].rows = 3;
	M[9].columns = 1;
	M[10].mat = &C[12];		/* CalculateJ\omega2 */
	M[10].rows = 3;
	M[10].columns = 1;
	M[11].mat = &C[15];		/* CalculateJ\omega3 */
	M[11].rows = 3;
	M[11].columns = 1;
	M[12].mat = &C[18];		/* CalculateJ\omega4 */
	M[12].rows = 3;
	M[12].columns = 1;
	M[13].mat = &C[21];		/* CalculateJ\omega5 */
	M[13].rows = 3;
	M[13].columns = 1;
	M[14].mat = &V[64];		/* CalculateJ\uTrzrefref */
	M[14].rows = 6;
	M[14].columns = 1;
	M[15].mat = &V[70];		/* CalculateJ\uTryrefref */
	M[15].rows = 6;
	M[15].columns = 1;
	M[16].mat = &V[76];		/* CalculateJ\uTrxrefref */
	M[16].rows = 6;
	M[16].columns = 1;
	M[17].mat = &V[82];		/* CalculateJ\uTr1refref */
	M[17].rows = 6;
	M[17].columns = 1;
	M[18].mat = &V[88];		/* CalculateJ\uTr211 */
	M[18].rows = 6;
	M[18].columns = 1;
	M[19].mat = &V[94];		/* CalculateJ\uTr322 */
	M[19].rows = 6;
	M[19].columns = 1;
	M[20].mat = &V[100];		/* CalculateJ\uTr433 */
	M[20].rows = 6;
	M[20].columns = 1;
	M[21].mat = &V[106];		/* CalculateJ\uTr544 */
	M[21].rows = 6;
	M[21].columns = 1;
	M[22].mat = &V[112];		/* CalculateJ\R1ref */
	M[22].rows = 3;
	M[22].columns = 3;
	M[23].mat = &V[121];		/* CalculateJ\R21 */
	M[23].rows = 3;
	M[23].columns = 3;
	M[24].mat = &V[130];		/* CalculateJ\R32 */
	M[24].rows = 3;
	M[24].columns = 3;
	M[25].mat = &V[139];		/* CalculateJ\R43 */
	M[25].rows = 3;
	M[25].columns = 3;
	M[26].mat = &V[148];		/* CalculateJ\R54 */
	M[26].rows = 3;
	M[26].columns = 3;
	M[27].mat = &V[157];		/* CalculateJ\Rtip5 */
	M[27].rows = 3;
	M[27].columns = 3;
	M[28].mat = &V[166];		/* CalculateJ\p1ref */
	M[28].rows = 3;
	M[28].columns = 1;
	M[29].mat = &V[169];		/* CalculateJ\p21 */
	M[29].rows = 3;
	M[29].columns = 1;
	M[30].mat = &V[172];		/* CalculateJ\p32 */
	M[30].rows = 3;
	M[30].columns = 1;
	M[31].mat = &V[175];		/* CalculateJ\p43 */
	M[31].rows = 3;
	M[31].columns = 1;
	M[32].mat = &V[178];		/* CalculateJ\p54 */
	M[32].rows = 3;
	M[32].columns = 1;
	M[33].mat = &V[181];		/* CalculateJ\ptip5 */
	M[33].rows = 3;
	M[33].columns = 1;
	M[34].mat = &V[184];		/* CalculateJ\Href0 */
	M[34].rows = 4;
	M[34].columns = 4;
	M[35].mat = &V[200];		/* CalculateJ\H1ref */
	M[35].rows = 4;
	M[35].columns = 4;
	M[36].mat = &V[216];		/* CalculateJ\H21 */
	M[36].rows = 4;
	M[36].columns = 4;
	M[37].mat = &V[232];		/* CalculateJ\H32 */
	M[37].rows = 4;
	M[37].columns = 4;
	M[38].mat = &V[248];		/* CalculateJ\H43 */
	M[38].rows = 4;
	M[38].columns = 4;
	M[39].mat = &V[264];		/* CalculateJ\H54 */
	M[39].rows = 4;
	M[39].columns = 4;
	M[40].mat = &V[280];		/* CalculateJ\Htip5 */
	M[40].rows = 4;
	M[40].columns = 4;
	M[41].mat = &V[296];		/* CalculateJ\H10 */
	M[41].rows = 4;
	M[41].columns = 4;
	M[42].mat = &V[312];		/* CalculateJ\H20 */
	M[42].rows = 4;
	M[42].columns = 4;
	M[43].mat = &V[328];		/* CalculateJ\H30 */
	M[43].rows = 4;
	M[43].columns = 4;
	M[44].mat = &V[344];		/* CalculateJ\H40 */
	M[44].rows = 4;
	M[44].columns = 4;
	M[45].mat = &V[360];		/* CalculateJ\H50 */
	M[45].rows = 4;
	M[45].columns = 4;
	M[46].mat = &V[376];		/* CalculateJ\base_offset_H */
	M[46].rows = 4;
	M[46].columns = 4;
	M[47].mat = &V[392];		/* CalculateJ\initialq */
	M[47].rows = 8;
	M[47].columns = 1;
	M[48].mat = &V[400];		/* CalculateJ\Rz */
	M[48].rows = 3;
	M[48].columns = 3;
	M[49].mat = &V[409];		/* CartesianSafetyFilter1\u */
	M[49].rows = 6;
	M[49].columns = 1;
	M[50].mat = &V[416];		/* CartesianSpaceStiffness\effort */
	M[50].rows = 6;
	M[50].columns = 1;
	M[51].mat = &P[43];		/* CartesianSpaceStiffness\H1_0_initial */
	M[51].rows = 4;
	M[51].columns = 4;
	M[52].mat = &P[59];		/* pose13 */
	M[52].rows = 4;
	M[52].columns = 4;
	M[53].mat = &P[75];		/* pose24 */
	M[53].rows = 4;
	M[53].columns = 4;
	M[54].mat = &P[91];		/* CartesianSpaceStiffness\stiffness */
	M[54].rows = 6;
	M[54].columns = 6;
	M[55].mat = &V[422];		/* CartesianSpaceStiffness\H1_0 */
	M[55].rows = 4;
	M[55].columns = 4;
	M[56].mat = &V[438];		/* CartesianSpaceStiffness\costiffness1 */
	M[56].rows = 3;
	M[56].columns = 3;
	M[57].mat = &V[447];		/* CartesianSpaceStiffness\costiffness2 */
	M[57].rows = 3;
	M[57].columns = 3;
	M[58].mat = &V[456];		/* CartesianSpaceStiffness\costiffness3 */
	M[58].rows = 3;
	M[58].columns = 3;
	M[59].mat = &V[465];		/* CartesianSpaceStiffness\dummy1 */
	M[59].rows = 3;
	M[59].columns = 3;
	M[60].mat = &V[474];		/* CartesianSpaceStiffness\dummy2 */
	M[60].rows = 3;
	M[60].columns = 3;
	M[61].mat = &V[483];		/* CartesianSpaceStiffness\dummy3 */
	M[61].rows = 3;
	M[61].columns = 3;
	M[62].mat = &V[492];		/* CartesianSpaceStiffness\dummy4 */
	M[62].rows = 3;
	M[62].columns = 3;
	M[63].mat = &V[501];		/* CartesianSpaceStiffness\dummy5 */
	M[63].rows = 3;
	M[63].columns = 3;
	M[64].mat = &V[510];		/* CartesianSpaceStiffness\skew21 */
	M[64].rows = 3;
	M[64].columns = 3;
	M[65].mat = &V[519];		/* CartesianSpaceStiffness\orientation21 */
	M[65].rows = 3;
	M[65].columns = 3;
	M[66].mat = &V[528];		/* Deadzone\output */
	M[66].rows = 8;
	M[66].columns = 1;
	M[67].mat = &P[127];		/* Deadzone\p_deadzone */
	M[67].rows = 8;
	M[67].columns = 1;
	M[68].mat = &P[135];		/* Deadzone\n_deadzone */
	M[68].rows = 8;
	M[68].columns = 1;
	M[69].mat = &V[536];		/* Gain1\H */
	M[69].rows = 4;
	M[69].columns = 4;
	M[70].mat = &V[552];		/* Gain1\Rz */
	M[70].rows = 3;
	M[70].columns = 3;
	M[71].mat = &V[561];		/* Gain1\Ry */
	M[71].rows = 3;
	M[71].columns = 3;
	M[72].mat = &V[570];		/* Gain1\Rx */
	M[72].rows = 3;
	M[72].columns = 3;
	M[73].mat = &V[579];		/* GravityCompensationModel\Base\AdHi0\p1.f */
	M[73].rows = 6;
	M[73].columns = 1;
	M[74].mat = &V[585];		/* GravityCompensationModel\Base\AdHi0\p2.e */
	M[74].rows = 6;
	M[74].columns = 1;
	M[75].mat = &V[591];		/* GravityCompensationModel\Base\AdHi0\onlyRotH */
	M[75].rows = 4;
	M[75].columns = 4;
	M[76].mat = &V[607];		/* GravityCompensationModel\Base\AdHij\p1.e */
	M[76].rows = 6;
	M[76].columns = 1;
	M[77].mat = &V[613];		/* GravityCompensationModel\Base\AdHij\p2.f */
	M[77].rows = 6;
	M[77].columns = 1;
	M[78].mat = &V[619];		/* GravityCompensationModel\Base\AdHik1\p1.e */
	M[78].rows = 6;
	M[78].columns = 1;
	M[79].mat = &V[625];		/* GravityCompensationModel\Base\AdHik1\p2.f */
	M[79].rows = 6;
	M[79].columns = 1;
	M[80].mat = &P[143];		/* GravityCompensationModel\Base\AdHik1\COMdim */
	M[80].rows = 3;
	M[80].columns = 1;
	M[81].mat = &V[631];		/* GravityCompensationModel\Base\AdHik1\d */
	M[81].rows = 3;
	M[81].columns = 1;
	M[82].mat = &V[634];		/* GravityCompensationModel\Base\AdHik1\R */
	M[82].rows = 3;
	M[82].columns = 3;
	M[83].mat = &V[643];		/* GravityCompensationModel\Base\AdHik1\H */
	M[83].rows = 4;
	M[83].columns = 4;
	M[84].mat = &V[659];		/* GravityCompensationModel\Base\AdHik1\AdH */
	M[84].rows = 6;
	M[84].columns = 6;
	M[85].mat = &V[695];		/* GravityCompensationModel\Base\Gravity\effort */
	M[85].rows = 6;
	M[85].columns = 1;
	M[86].mat = &V[701];		/* GravityCompensationModel\Base\Hij\output */
	M[86].rows = 4;
	M[86].columns = 4;
	M[87].mat = &V[717];		/* GravityCompensationModel\Base\Hij\Hab */
	M[87].rows = 4;
	M[87].columns = 4;
	M[88].mat = &P[147];		/* GravityCompensationModel\Base\Hij\dim */
	M[88].rows = 3;
	M[88].columns = 1;
	M[89].mat = &V[733];		/* GravityCompensationModel\Base\Ta0i\p2.e */
	M[89].rows = 6;
	M[89].columns = 1;
	M[90].mat = &P[151];		/* GravityCompensationModel\Base\COMdim */
	M[90].rows = 3;
	M[90].columns = 1;
	M[91].mat = &P[154];		/* GravityCompensationModel\Base\dim */
	M[91].rows = 3;
	M[91].columns = 1;
	M[92].mat = &V[739];		/* GravityCompensationModel\Hfloor0\output */
	M[92].rows = 4;
	M[92].columns = 4;
	M[93].mat = &V[755];		/* GravityCompensationModel\Joint12\AdHji\p1.e */
	M[93].rows = 6;
	M[93].columns = 1;
	M[94].mat = &V[761];		/* GravityCompensationModel\Joint12\AdHji\p2.f */
	M[94].rows = 6;
	M[94].columns = 1;
	M[95].mat = &V[767];		/* GravityCompensationModel\Joint12\Integrate\output */
	M[95].rows = 4;
	M[95].columns = 4;
	M[96].mat = &V[783];		/* GravityCompensationModel\Joint12\Integrate\R */
	M[96].rows = 3;
	M[96].columns = 3;
	M[97].mat = &V[792];		/* GravityCompensationModel\Joint12\Integrate\p */
	M[97].rows = 3;
	M[97].columns = 1;
	M[98].mat = &V[795];		/* GravityCompensationModel\Joint12\MatrixMul\output */
	M[98].rows = 4;
	M[98].columns = 4;
	M[99].mat = &V[812];		/* GravityCompensationModel\Joint12\uTbai\p2.f */
	M[99].rows = 6;
	M[99].columns = 1;
	M[100].mat = &V[818];		/* GravityCompensationModel\Joint12\Wbai\p3.f */
	M[100].rows = 6;
	M[100].columns = 1;
	M[101].mat = &V[824];		/* GravityCompensationModel\Joint23\AdHji\p1.e */
	M[101].rows = 6;
	M[101].columns = 1;
	M[102].mat = &V[830];		/* GravityCompensationModel\Joint23\AdHji\p2.f */
	M[102].rows = 6;
	M[102].columns = 1;
	M[103].mat = &V[836];		/* GravityCompensationModel\Joint23\Integrate\output */
	M[103].rows = 4;
	M[103].columns = 4;
	M[104].mat = &V[852];		/* GravityCompensationModel\Joint23\Integrate\R */
	M[104].rows = 3;
	M[104].columns = 3;
	M[105].mat = &V[861];		/* GravityCompensationModel\Joint23\Integrate\p */
	M[105].rows = 3;
	M[105].columns = 1;
	M[106].mat = &V[864];		/* GravityCompensationModel\Joint23\MatrixMul\output */
	M[106].rows = 4;
	M[106].columns = 4;
	M[107].mat = &V[881];		/* GravityCompensationModel\Joint23\uTbai\p2.f */
	M[107].rows = 6;
	M[107].columns = 1;
	M[108].mat = &V[887];		/* GravityCompensationModel\Joint23\Wbai\p3.f */
	M[108].rows = 6;
	M[108].columns = 1;
	M[109].mat = &V[893];		/* GravityCompensationModel\Joint34\AdHji\p1.e */
	M[109].rows = 6;
	M[109].columns = 1;
	M[110].mat = &V[899];		/* GravityCompensationModel\Joint34\AdHji\p2.f */
	M[110].rows = 6;
	M[110].columns = 1;
	M[111].mat = &V[905];		/* GravityCompensationModel\Joint34\Integrate\output */
	M[111].rows = 4;
	M[111].columns = 4;
	M[112].mat = &V[921];		/* GravityCompensationModel\Joint34\Integrate\R */
	M[112].rows = 3;
	M[112].columns = 3;
	M[113].mat = &V[930];		/* GravityCompensationModel\Joint34\Integrate\p */
	M[113].rows = 3;
	M[113].columns = 1;
	M[114].mat = &V[933];		/* GravityCompensationModel\Joint34\MatrixMul\output */
	M[114].rows = 4;
	M[114].columns = 4;
	M[115].mat = &V[950];		/* GravityCompensationModel\Joint34\uTbai\p2.f */
	M[115].rows = 6;
	M[115].columns = 1;
	M[116].mat = &V[956];		/* GravityCompensationModel\Joint34\Wbai\p3.f */
	M[116].rows = 6;
	M[116].columns = 1;
	M[117].mat = &V[962];		/* GravityCompensationModel\Joint45\AdHji\p1.e */
	M[117].rows = 6;
	M[117].columns = 1;
	M[118].mat = &V[968];		/* GravityCompensationModel\Joint45\AdHji\p2.f */
	M[118].rows = 6;
	M[118].columns = 1;
	M[119].mat = &V[974];		/* GravityCompensationModel\Joint45\Integrate\output */
	M[119].rows = 4;
	M[119].columns = 4;
	M[120].mat = &V[990];		/* GravityCompensationModel\Joint45\Integrate\R */
	M[120].rows = 3;
	M[120].columns = 3;
	M[121].mat = &V[999];		/* GravityCompensationModel\Joint45\Integrate\p */
	M[121].rows = 3;
	M[121].columns = 1;
	M[122].mat = &V[1002];		/* GravityCompensationModel\Joint45\MatrixMul\output */
	M[122].rows = 4;
	M[122].columns = 4;
	M[123].mat = &V[1019];		/* GravityCompensationModel\Joint45\uTbai\p2.f */
	M[123].rows = 6;
	M[123].columns = 1;
	M[124].mat = &V[1025];		/* GravityCompensationModel\Joint45\Wbai\p1.f */
	M[124].rows = 6;
	M[124].columns = 1;
	M[125].mat = &V[1031];		/* GravityCompensationModel\JointBase1\AdHji\p1.e */
	M[125].rows = 6;
	M[125].columns = 1;
	M[126].mat = &V[1037];		/* GravityCompensationModel\JointBase1\AdHji\p2.f */
	M[126].rows = 6;
	M[126].columns = 1;
	M[127].mat = &V[1043];		/* GravityCompensationModel\JointBase1\Integrate\output */
	M[127].rows = 4;
	M[127].columns = 4;
	M[128].mat = &V[1059];		/* GravityCompensationModel\JointBase1\Integrate\R */
	M[128].rows = 3;
	M[128].columns = 3;
	M[129].mat = &V[1068];		/* GravityCompensationModel\JointBase1\Integrate\p */
	M[129].rows = 3;
	M[129].columns = 1;
	M[130].mat = &V[1071];		/* GravityCompensationModel\JointBase1\MatrixMul\output */
	M[130].rows = 4;
	M[130].columns = 4;
	M[131].mat = &V[1088];		/* GravityCompensationModel\JointBase1\uTbai\p2.f */
	M[131].rows = 6;
	M[131].columns = 1;
	M[132].mat = &V[1094];		/* GravityCompensationModel\JointBase1\Wbai\p1.f */
	M[132].rows = 6;
	M[132].columns = 1;
	M[133].mat = &V[1100];		/* GravityCompensationModel\Link1\AdHi0\p1.f */
	M[133].rows = 6;
	M[133].columns = 1;
	M[134].mat = &V[1106];		/* GravityCompensationModel\Link1\AdHi0\p2.e */
	M[134].rows = 6;
	M[134].columns = 1;
	M[135].mat = &V[1112];		/* GravityCompensationModel\Link1\AdHi0\onlyRotH */
	M[135].rows = 4;
	M[135].columns = 4;
	M[136].mat = &V[1128];		/* GravityCompensationModel\Link1\AdHij\p1.e */
	M[136].rows = 6;
	M[136].columns = 1;
	M[137].mat = &V[1134];		/* GravityCompensationModel\Link1\AdHij\p2.f */
	M[137].rows = 6;
	M[137].columns = 1;
	M[138].mat = &V[1140];		/* GravityCompensationModel\Link1\AdHik1\p1.e */
	M[138].rows = 6;
	M[138].columns = 1;
	M[139].mat = &V[1146];		/* GravityCompensationModel\Link1\AdHik1\p2.f */
	M[139].rows = 6;
	M[139].columns = 1;
	M[140].mat = &P[167];		/* GravityCompensationModel\Link1\AdHik1\COMdim */
	M[140].rows = 3;
	M[140].columns = 1;
	M[141].mat = &V[1152];		/* GravityCompensationModel\Link1\AdHik1\d */
	M[141].rows = 3;
	M[141].columns = 1;
	M[142].mat = &V[1155];		/* GravityCompensationModel\Link1\AdHik1\R */
	M[142].rows = 3;
	M[142].columns = 3;
	M[143].mat = &V[1164];		/* GravityCompensationModel\Link1\AdHik1\H */
	M[143].rows = 4;
	M[143].columns = 4;
	M[144].mat = &V[1180];		/* GravityCompensationModel\Link1\AdHik1\AdH */
	M[144].rows = 6;
	M[144].columns = 6;
	M[145].mat = &V[1216];		/* GravityCompensationModel\Link1\Gravity\effort */
	M[145].rows = 6;
	M[145].columns = 1;
	M[146].mat = &V[1222];		/* GravityCompensationModel\Link1\Hij\output */
	M[146].rows = 4;
	M[146].columns = 4;
	M[147].mat = &V[1238];		/* GravityCompensationModel\Link1\Hij\Hab */
	M[147].rows = 4;
	M[147].columns = 4;
	M[148].mat = &P[171];		/* GravityCompensationModel\Link1\Hij\dim */
	M[148].rows = 3;
	M[148].columns = 1;
	M[149].mat = &V[1254];		/* GravityCompensationModel\Link1\Ta0i\p2.e */
	M[149].rows = 6;
	M[149].columns = 1;
	M[150].mat = &P[175];		/* GravityCompensationModel\Link1\COMdim */
	M[150].rows = 3;
	M[150].columns = 1;
	M[151].mat = &P[178];		/* GravityCompensationModel\Link1\dim */
	M[151].rows = 3;
	M[151].columns = 1;
	M[152].mat = &V[1260];		/* GravityCompensationModel\Link2\AdHi0\p1.f */
	M[152].rows = 6;
	M[152].columns = 1;
	M[153].mat = &V[1266];		/* GravityCompensationModel\Link2\AdHi0\p2.e */
	M[153].rows = 6;
	M[153].columns = 1;
	M[154].mat = &V[1272];		/* GravityCompensationModel\Link2\AdHi0\onlyRotH */
	M[154].rows = 4;
	M[154].columns = 4;
	M[155].mat = &V[1288];		/* GravityCompensationModel\Link2\AdHij\p1.e */
	M[155].rows = 6;
	M[155].columns = 1;
	M[156].mat = &V[1294];		/* GravityCompensationModel\Link2\AdHij\p2.f */
	M[156].rows = 6;
	M[156].columns = 1;
	M[157].mat = &V[1300];		/* GravityCompensationModel\Link2\AdHik1\p1.e */
	M[157].rows = 6;
	M[157].columns = 1;
	M[158].mat = &V[1306];		/* GravityCompensationModel\Link2\AdHik1\p2.f */
	M[158].rows = 6;
	M[158].columns = 1;
	M[159].mat = &P[181];		/* GravityCompensationModel\Link2\AdHik1\COMdim */
	M[159].rows = 3;
	M[159].columns = 1;
	M[160].mat = &V[1312];		/* GravityCompensationModel\Link2\AdHik1\d */
	M[160].rows = 3;
	M[160].columns = 1;
	M[161].mat = &V[1315];		/* GravityCompensationModel\Link2\AdHik1\R */
	M[161].rows = 3;
	M[161].columns = 3;
	M[162].mat = &V[1324];		/* GravityCompensationModel\Link2\AdHik1\H */
	M[162].rows = 4;
	M[162].columns = 4;
	M[163].mat = &V[1340];		/* GravityCompensationModel\Link2\AdHik1\AdH */
	M[163].rows = 6;
	M[163].columns = 6;
	M[164].mat = &V[1376];		/* GravityCompensationModel\Link2\Gravity\effort */
	M[164].rows = 6;
	M[164].columns = 1;
	M[165].mat = &V[1382];		/* GravityCompensationModel\Link2\Hij\output */
	M[165].rows = 4;
	M[165].columns = 4;
	M[166].mat = &V[1398];		/* GravityCompensationModel\Link2\Hij\Hab */
	M[166].rows = 4;
	M[166].columns = 4;
	M[167].mat = &P[185];		/* GravityCompensationModel\Link2\Hij\dim */
	M[167].rows = 3;
	M[167].columns = 1;
	M[168].mat = &V[1414];		/* GravityCompensationModel\Link2\Ta0i\p2.e */
	M[168].rows = 6;
	M[168].columns = 1;
	M[169].mat = &P[189];		/* GravityCompensationModel\Link2\COMdim */
	M[169].rows = 3;
	M[169].columns = 1;
	M[170].mat = &P[192];		/* GravityCompensationModel\Link2\dim */
	M[170].rows = 3;
	M[170].columns = 1;
	M[171].mat = &V[1420];		/* GravityCompensationModel\Link3\AdHi0\p1.f */
	M[171].rows = 6;
	M[171].columns = 1;
	M[172].mat = &V[1426];		/* GravityCompensationModel\Link3\AdHi0\p2.e */
	M[172].rows = 6;
	M[172].columns = 1;
	M[173].mat = &V[1432];		/* GravityCompensationModel\Link3\AdHi0\onlyRotH */
	M[173].rows = 4;
	M[173].columns = 4;
	M[174].mat = &V[1448];		/* GravityCompensationModel\Link3\AdHij\p1.e */
	M[174].rows = 6;
	M[174].columns = 1;
	M[175].mat = &V[1454];		/* GravityCompensationModel\Link3\AdHij\p2.f */
	M[175].rows = 6;
	M[175].columns = 1;
	M[176].mat = &V[1460];		/* GravityCompensationModel\Link3\AdHik1\p1.e */
	M[176].rows = 6;
	M[176].columns = 1;
	M[177].mat = &V[1466];		/* GravityCompensationModel\Link3\AdHik1\p2.f */
	M[177].rows = 6;
	M[177].columns = 1;
	M[178].mat = &P[195];		/* GravityCompensationModel\Link3\AdHik1\COMdim */
	M[178].rows = 3;
	M[178].columns = 1;
	M[179].mat = &V[1472];		/* GravityCompensationModel\Link3\AdHik1\d */
	M[179].rows = 3;
	M[179].columns = 1;
	M[180].mat = &V[1475];		/* GravityCompensationModel\Link3\AdHik1\R */
	M[180].rows = 3;
	M[180].columns = 3;
	M[181].mat = &V[1484];		/* GravityCompensationModel\Link3\AdHik1\H */
	M[181].rows = 4;
	M[181].columns = 4;
	M[182].mat = &V[1500];		/* GravityCompensationModel\Link3\AdHik1\AdH */
	M[182].rows = 6;
	M[182].columns = 6;
	M[183].mat = &V[1536];		/* GravityCompensationModel\Link3\Gravity\effort */
	M[183].rows = 6;
	M[183].columns = 1;
	M[184].mat = &V[1542];		/* GravityCompensationModel\Link3\Hij\output */
	M[184].rows = 4;
	M[184].columns = 4;
	M[185].mat = &V[1558];		/* GravityCompensationModel\Link3\Hij\Hab */
	M[185].rows = 4;
	M[185].columns = 4;
	M[186].mat = &P[199];		/* GravityCompensationModel\Link3\Hij\dim */
	M[186].rows = 3;
	M[186].columns = 1;
	M[187].mat = &V[1574];		/* GravityCompensationModel\Link3\Ta0i\p2.e */
	M[187].rows = 6;
	M[187].columns = 1;
	M[188].mat = &P[203];		/* GravityCompensationModel\Link3\COMdim */
	M[188].rows = 3;
	M[188].columns = 1;
	M[189].mat = &P[206];		/* GravityCompensationModel\Link3\dim */
	M[189].rows = 3;
	M[189].columns = 1;
	M[190].mat = &V[1580];		/* GravityCompensationModel\Link4\AdHi0\p1.f */
	M[190].rows = 6;
	M[190].columns = 1;
	M[191].mat = &V[1586];		/* GravityCompensationModel\Link4\AdHi0\p2.e */
	M[191].rows = 6;
	M[191].columns = 1;
	M[192].mat = &V[1592];		/* GravityCompensationModel\Link4\AdHi0\onlyRotH */
	M[192].rows = 4;
	M[192].columns = 4;
	M[193].mat = &V[1608];		/* GravityCompensationModel\Link4\AdHij\p1.e */
	M[193].rows = 6;
	M[193].columns = 1;
	M[194].mat = &V[1614];		/* GravityCompensationModel\Link4\AdHij\p2.f */
	M[194].rows = 6;
	M[194].columns = 1;
	M[195].mat = &V[1620];		/* GravityCompensationModel\Link4\AdHik1\p1.e */
	M[195].rows = 6;
	M[195].columns = 1;
	M[196].mat = &V[1626];		/* GravityCompensationModel\Link4\AdHik1\p2.f */
	M[196].rows = 6;
	M[196].columns = 1;
	M[197].mat = &P[209];		/* GravityCompensationModel\Link4\AdHik1\COMdim */
	M[197].rows = 3;
	M[197].columns = 1;
	M[198].mat = &V[1632];		/* GravityCompensationModel\Link4\AdHik1\d */
	M[198].rows = 3;
	M[198].columns = 1;
	M[199].mat = &V[1635];		/* GravityCompensationModel\Link4\AdHik1\R */
	M[199].rows = 3;
	M[199].columns = 3;
	M[200].mat = &V[1644];		/* GravityCompensationModel\Link4\AdHik1\H */
	M[200].rows = 4;
	M[200].columns = 4;
	M[201].mat = &V[1660];		/* GravityCompensationModel\Link4\AdHik1\AdH */
	M[201].rows = 6;
	M[201].columns = 6;
	M[202].mat = &V[1696];		/* GravityCompensationModel\Link4\Gravity\effort */
	M[202].rows = 6;
	M[202].columns = 1;
	M[203].mat = &V[1702];		/* GravityCompensationModel\Link4\Hij\output */
	M[203].rows = 4;
	M[203].columns = 4;
	M[204].mat = &V[1718];		/* GravityCompensationModel\Link4\Hij\Hab */
	M[204].rows = 4;
	M[204].columns = 4;
	M[205].mat = &P[213];		/* GravityCompensationModel\Link4\Hij\dim */
	M[205].rows = 3;
	M[205].columns = 1;
	M[206].mat = &V[1734];		/* GravityCompensationModel\Link4\Ta0i\p2.e */
	M[206].rows = 6;
	M[206].columns = 1;
	M[207].mat = &P[217];		/* GravityCompensationModel\Link4\COMdim */
	M[207].rows = 3;
	M[207].columns = 1;
	M[208].mat = &P[220];		/* GravityCompensationModel\Link4\dim */
	M[208].rows = 3;
	M[208].columns = 1;
	M[209].mat = &V[1740];		/* GravityCompensationModel\Link5\AdHi0\p1.f */
	M[209].rows = 6;
	M[209].columns = 1;
	M[210].mat = &V[1746];		/* GravityCompensationModel\Link5\AdHi0\p2.e */
	M[210].rows = 6;
	M[210].columns = 1;
	M[211].mat = &V[1752];		/* GravityCompensationModel\Link5\AdHi0\onlyRotH */
	M[211].rows = 4;
	M[211].columns = 4;
	M[212].mat = &V[1768];		/* GravityCompensationModel\Link5\AdHij\p1.e */
	M[212].rows = 6;
	M[212].columns = 1;
	M[213].mat = &V[1774];		/* GravityCompensationModel\Link5\AdHij\p2.f */
	M[213].rows = 6;
	M[213].columns = 1;
	M[214].mat = &V[1780];		/* GravityCompensationModel\Link5\AdHik1\p1.e */
	M[214].rows = 6;
	M[214].columns = 1;
	M[215].mat = &V[1786];		/* GravityCompensationModel\Link5\AdHik1\p2.f */
	M[215].rows = 6;
	M[215].columns = 1;
	M[216].mat = &P[223];		/* GravityCompensationModel\Link5\AdHik1\COMdim */
	M[216].rows = 3;
	M[216].columns = 1;
	M[217].mat = &V[1792];		/* GravityCompensationModel\Link5\AdHik1\d */
	M[217].rows = 3;
	M[217].columns = 1;
	M[218].mat = &V[1795];		/* GravityCompensationModel\Link5\AdHik1\R */
	M[218].rows = 3;
	M[218].columns = 3;
	M[219].mat = &V[1804];		/* GravityCompensationModel\Link5\AdHik1\H */
	M[219].rows = 4;
	M[219].columns = 4;
	M[220].mat = &V[1820];		/* GravityCompensationModel\Link5\AdHik1\AdH */
	M[220].rows = 6;
	M[220].columns = 6;
	M[221].mat = &V[1856];		/* GravityCompensationModel\Link5\Gravity\effort */
	M[221].rows = 6;
	M[221].columns = 1;
	M[222].mat = &V[1862];		/* GravityCompensationModel\Link5\Hij\output */
	M[222].rows = 4;
	M[222].columns = 4;
	M[223].mat = &V[1878];		/* GravityCompensationModel\Link5\Hij\Hab */
	M[223].rows = 4;
	M[223].columns = 4;
	M[224].mat = &P[227];		/* GravityCompensationModel\Link5\Hij\dim */
	M[224].rows = 3;
	M[224].columns = 1;
	M[225].mat = &V[1894];		/* GravityCompensationModel\Link5\Ta0i\p2.e */
	M[225].rows = 6;
	M[225].columns = 1;
	M[226].mat = &P[231];		/* GravityCompensationModel\Link5\COMdim */
	M[226].rows = 3;
	M[226].columns = 1;
	M[227].mat = &P[234];		/* GravityCompensationModel\Link5\dim */
	M[227].rows = 3;
	M[227].columns = 1;
	M[228].mat = &V[1900];		/* GravityCompensationModel\Mux\port.e */
	M[228].rows = 8;
	M[228].columns = 1;
	M[229].mat = &V[1918];		/* GravityCompensationModel\Wtip0\p.e */
	M[229].rows = 6;
	M[229].columns = 1;
	M[230].mat = &V[1924];		/* GravityCompensationModel\ZeroJunction\p1.f */
	M[230].rows = 6;
	M[230].columns = 1;
	M[231].mat = &V[1930];		/* GravityCompensationModel\ZeroJunction1\p1.f */
	M[231].rows = 8;
	M[231].columns = 1;
	M[232].mat = &P[239];		/* GravityCompensationModel\InitialJointPos */
	M[232].rows = 5;
	M[232].columns = 1;
	M[233].mat = &P[244];		/* GravityCompensationModel\LinkCOMdim */
	M[233].rows = 3;
	M[233].columns = 6;
	M[234].mat = &P[262];		/* GravityCompensationModel\Linkdim */
	M[234].rows = 3;
	M[234].columns = 6;
	M[235].mat = &P[280];		/* GravityCompensationModel\Linkm */
	M[235].rows = 6;
	M[235].columns = 1;
	M[236].mat = &V[1938];		/* JointSoftLimits\output */
	M[236].rows = 8;
	M[236].columns = 1;
	M[237].mat = &P[286];		/* JointSoftLimits\c */
	M[237].rows = 8;
	M[237].columns = 1;
	M[238].mat = &P[294];		/* JointSoftLimits\limits_low */
	M[238].rows = 8;
	M[238].columns = 1;
	M[239].mat = &P[302];		/* JointSoftLimits\limits_high */
	M[239].rows = 8;
	M[239].columns = 1;
	M[240].mat = &V[1946];		/* JointSoftLimits\substate */
	M[240].rows = 8;
	M[240].columns = 1;
	M[241].mat = &V[1954];		/* JointSoftLimits\angle_limits_low */
	M[241].rows = 8;
	M[241].columns = 1;
	M[242].mat = &V[1962];		/* JointSoftLimits\angle_limits_high */
	M[242].rows = 8;
	M[242].columns = 1;
	M[243].mat = &V[1970];		/* JointSpaceDamping\output */
	M[243].rows = 8;
	M[243].columns = 1;
	M[244].mat = &P[311];		/* JointSpaceDamping\r */
	M[244].rows = 8;
	M[244].columns = 1;
	M[245].mat = &V[1978];		/* JointSpaceDamping\R */
	M[245].rows = 8;
	M[245].columns = 8;
	M[246].mat = &V[2042];		/* JointSpaceStiffness\output */
	M[246].rows = 8;
	M[246].columns = 1;
	M[247].mat = &P[319];		/* JointSpaceStiffness\c */
	M[247].rows = 8;
	M[247].columns = 1;
	M[248].mat = &V[2050];		/* Limit1\output */
	M[248].rows = 8;
	M[248].columns = 1;
	M[249].mat = &V[2058];		/* Limit3\output */
	M[249].rows = 8;
	M[249].columns = 1;
	M[250].mat = &V[2066];		/* Mux\joint_states */
	M[250].rows = 8;
	M[250].columns = 1;
	M[251].mat = &V[2074];		/* Mux\Arm1_joint_states */
	M[251].rows = 5;
	M[251].columns = 1;
	M[252].mat = &V[2079];		/* Mux\measured_odometry */
	M[252].rows = 3;
	M[252].columns = 1;
	M[253].mat = &V[2082];		/* Mux\Arm1_joint_velocites */
	M[253].rows = 5;
	M[253].columns = 1;
	M[254].mat = &V[2087];		/* Mux\joints_velocities */
	M[254].rows = 8;
	M[254].columns = 1;
	M[255].mat = &V[2095];		/* Mux\base_joint_velocities */
	M[255].rows = 4;
	M[255].columns = 1;
	M[256].mat = &V[2099];		/* Mux\Arm1_joint_cmd */
	M[256].rows = 5;
	M[256].columns = 1;
	M[257].mat = &V[2104];		/* Mux\Base_twist_cmd */
	M[257].rows = 3;
	M[257].columns = 1;
	M[258].mat = &P[330];		/* Mux\joints_initial_states */
	M[258].rows = 5;
	M[258].columns = 1;
	M[259].mat = &V[2107];		/* PlusMinus1\output */
	M[259].rows = 8;
	M[259].columns = 1;
	M[260].mat = &V[2115];		/* PlusMinus2\output */
	M[260].rows = 8;
	M[260].columns = 1;
	M[261].mat = &V[2123];		/* PlusMinus2\plus1 */
	M[261].rows = 8;
	M[261].columns = 1;
	M[262].mat = &V[2131];		/* PlusMinus3\output */
	M[262].rows = 8;
	M[262].columns = 1;
	M[263].mat = &V[2139];		/* PlusMinus4\output */
	M[263].rows = 8;
	M[263].columns = 1;
	M[264].mat = &V[2147];		/* PlusMinus5\output */
	M[264].rows = 8;
	M[264].columns = 1;
	M[265].mat = &V[2155];		/* TF\joints_e */
	M[265].rows = 8;
	M[265].columns = 1;
	M[266].mat = &V[2163];		/* Measured_odometry */
	M[266].rows = 3;
	M[266].columns = 1;
	M[267].mat = &V[2166];		/* Arm1_joint_velocites */
	M[267].rows = 5;
	M[267].columns = 1;
	M[268].mat = &V[2171];		/* Base_joint_velocities */
	M[268].rows = 4;
	M[268].columns = 1;
	M[269].mat = &V[2175];		/* xyzypr */
	M[269].rows = 6;
	M[269].columns = 1;
	M[270].mat = &V[2181];		/* Htip0 */
	M[270].rows = 4;
	M[270].columns = 4;
	M[271].mat = &V[2197];		/* Arm1_joint_cmd */
	M[271].rows = 5;
	M[271].columns = 1;
	M[272].mat = &V[2202];		/* Base_twist_cmd */
	M[272].rows = 3;
	M[272].columns = 1;
	M[273].mat = &V[2205];		/* JointPoistionSetpoint */
	M[273].rows = 8;
	M[273].columns = 1;
	M[274].mat = &V[2213];		/* Arm1_joint_states */
	M[274].rows = 5;
	M[274].columns = 1;
	M[275].mat = &R[0];		/* CartesianSafetyFilter1\s2y */
	M[275].rows = 6;
	M[275].columns = 1;
	M[276].mat = &I[0];		/* CartesianSafetyFilter1\s1y_initial */
	M[276].rows = 6;
	M[276].columns = 1;
	M[277].mat = &s[0];		/* CartesianSafetyFilter1\s1y */
	M[277].rows = 6;
	M[277].columns = 1;
	M[278].mat = &s[6];		/* CartesianSafetyFilter1\y */
	M[278].rows = 6;
	M[278].columns = 1;
	M[279].mat = &I[6];		/* CartesianSafetyFilter1\initPos */
	M[279].rows = 6;
	M[279].columns = 1;
	M[280].mat = &U[0];		/* U1 */
	M[280].rows = 3;
	M[280].columns = 3;
	M[281].mat = &U[9];		/* U2 */
	M[281].rows = 3;
	M[281].columns = 3;
	M[282].mat = &U[18];		/* U3 */
	M[282].rows = 3;
	M[282].columns = 3;
	M[283].mat = &U[27];		/* U4 */
	M[283].rows = 3;
	M[283].columns = 3;
	M[284].mat = &U[36];		/* U5 */
	M[284].rows = 3;
	M[284].columns = 3;
	M[285].mat = &U[45];		/* U6 */
	M[285].rows = 3;
	M[285].columns = 3;
	M[286].mat = &U[54];		/* U7 */
	M[286].rows = 3;
	M[286].columns = 3;
	M[287].mat = &U[63];		/* U8 */
	M[287].rows = 3;
	M[287].columns = 1;
	M[288].mat = &U[66];		/* U9 */
	M[288].rows = 3;
	M[288].columns = 3;
	M[289].mat = &U[75];		/* U10 */
	M[289].rows = 3;
	M[289].columns = 1;
	M[290].mat = &U[78];		/* U11 */
	M[290].rows = 3;
	M[290].columns = 1;
	M[291].mat = &U[81];		/* U12 */
	M[291].rows = 3;
	M[291].columns = 3;
	M[292].mat = &U[90];		/* U13 */
	M[292].rows = 3;
	M[292].columns = 1;
	M[293].mat = &U[93];		/* U14 */
	M[293].rows = 3;
	M[293].columns = 1;
	M[294].mat = &U[96];		/* U15 */
	M[294].rows = 3;
	M[294].columns = 3;
	M[295].mat = &U[105];		/* U16 */
	M[295].rows = 3;
	M[295].columns = 1;
	M[296].mat = &U[108];		/* U17 */
	M[296].rows = 3;
	M[296].columns = 1;
	M[297].mat = &U[111];		/* U18 */
	M[297].rows = 3;
	M[297].columns = 3;
	M[298].mat = &U[120];		/* U19 */
	M[298].rows = 3;
	M[298].columns = 1;
	M[299].mat = &U[123];		/* U20 */
	M[299].rows = 3;
	M[299].columns = 1;
	M[300].mat = &U[126];		/* U21 */
	M[300].rows = 3;
	M[300].columns = 3;
	M[301].mat = &U[135];		/* U22 */
	M[301].rows = 3;
	M[301].columns = 1;
	M[302].mat = &U[138];		/* U23 */
	M[302].rows = 3;
	M[302].columns = 3;
	M[303].mat = &U[147];		/* U24 */
	M[303].rows = 3;
	M[303].columns = 3;
	M[304].mat = &U[156];		/* U25 */
	M[304].rows = 3;
	M[304].columns = 3;
	M[305].mat = &U[165];		/* U26 */
	M[305].rows = 3;
	M[305].columns = 3;
	M[306].mat = &U[174];		/* U27 */
	M[306].rows = 3;
	M[306].columns = 3;
	M[307].mat = &U[183];		/* U28 */
	M[307].rows = 3;
	M[307].columns = 3;
	M[308].mat = &U[192];		/* U29 */
	M[308].rows = 6;
	M[308].columns = 6;
	M[309].mat = &U[228];		/* U30 */
	M[309].rows = 6;
	M[309].columns = 6;
	M[310].mat = &U[264];		/* U31 */
	M[310].rows = 6;
	M[310].columns = 6;
	M[311].mat = &U[300];		/* U32 */
	M[311].rows = 6;
	M[311].columns = 6;
	M[312].mat = &U[336];		/* U33 */
	M[312].rows = 6;
	M[312].columns = 1;
	M[313].mat = &U[342];		/* U34 */
	M[313].rows = 6;
	M[313].columns = 1;
	M[314].mat = &U[348];		/* U35 */
	M[314].rows = 6;
	M[314].columns = 1;
	M[315].mat = &U[354];		/* U36 */
	M[315].rows = 3;
	M[315].columns = 3;
	M[316].mat = &U[363];		/* U37 */
	M[316].rows = 3;
	M[316].columns = 3;
	M[317].mat = &U[372];		/* U38 */
	M[317].rows = 3;
	M[317].columns = 1;
	M[318].mat = &U[375];		/* U39 */
	M[318].rows = 5;
	M[318].columns = 1;
	M[319].mat = &U[380];		/* U40 */
	M[319].rows = 6;
	M[319].columns = 6;
	M[320].mat = &U[416];		/* U41 */
	M[320].rows = 6;
	M[320].columns = 6;
	M[321].mat = &U[452];		/* U42 */
	M[321].rows = 6;
	M[321].columns = 6;
	M[322].mat = &U[488];		/* U43 */
	M[322].rows = 8;
	M[322].columns = 8;
	M[323].mat = &U[552];		/* U44 */
	M[323].rows = 8;
	M[323].columns = 8;
	M[324].mat = &U[616];		/* U45 */
	M[324].rows = 8;
	M[324].columns = 8;
	M[325].mat = &U[680];		/* U46 */
	M[325].rows = 4;
	M[325].columns = 4;
	M[326].mat = &U[696];		/* U47 */
	M[326].rows = 3;
	M[326].columns = 1;
	M[327].mat = &U[699];		/* U48 */
	M[327].rows = 6;
	M[327].columns = 1;
	M[328].mat = &U[705];		/* U49 */
	M[328].rows = 6;
	M[328].columns = 6;
	M[329].mat = &U[741];		/* U50 */
	M[329].rows = 6;
	M[329].columns = 1;
	M[330].mat = &U[747];		/* U51 */
	M[330].rows = 6;
	M[330].columns = 6;
	M[331].mat = &U[783];		/* U52 */
	M[331].rows = 6;
	M[331].columns = 1;
	M[332].mat = &U[789];		/* U53 */
	M[332].rows = 6;
	M[332].columns = 6;
	M[333].mat = &U[825];		/* U54 */
	M[333].rows = 6;
	M[333].columns = 1;
	M[334].mat = &U[831];		/* U55 */
	M[334].rows = 6;
	M[334].columns = 6;
	M[335].mat = &U[867];		/* U56 */
	M[335].rows = 6;
	M[335].columns = 1;
	M[336].mat = &U[873];		/* U57 */
	M[336].rows = 6;
	M[336].columns = 6;
	M[337].mat = &U[909];		/* U58 */
	M[337].rows = 6;
	M[337].columns = 1;
	M[338].mat = &U[915];		/* U59 */
	M[338].rows = 6;
	M[338].columns = 6;
	M[339].mat = &U[951];		/* U60 */
	M[339].rows = 6;
	M[339].columns = 1;
	M[340].mat = &U[957];		/* U61 */
	M[340].rows = 6;
	M[340].columns = 6;
	M[341].mat = &U[993];		/* U62 */
	M[341].rows = 6;
	M[341].columns = 1;
	M[342].mat = &U[999];		/* U63 */
	M[342].rows = 6;
	M[342].columns = 6;
	M[343].mat = &U[1035];		/* U64 */
	M[343].rows = 8;
	M[343].columns = 1;
	M[344].mat = &U[1043];		/* U65 */
	M[344].rows = 8;
	M[344].columns = 8;
	M[345].mat = &U[1107];		/* U66 */
	M[345].rows = 4;
	M[345].columns = 4;
	M[346].mat = &U[1123];		/* U67 */
	M[346].rows = 3;
	M[346].columns = 1;
	M[347].mat = &U[1126];		/* U68 */
	M[347].rows = 3;
	M[347].columns = 3;
	M[348].mat = &U[1135];		/* U69 */
	M[348].rows = 3;
	M[348].columns = 3;
	M[349].mat = &U[1144];		/* U70 */
	M[349].rows = 3;
	M[349].columns = 3;
	M[350].mat = &U[1153];		/* U71 */
	M[350].rows = 3;
	M[350].columns = 3;
	M[351].mat = &U[1162];		/* U72 */
	M[351].rows = 3;
	M[351].columns = 3;
	M[352].mat = &U[1171];		/* U73 */
	M[352].rows = 3;
	M[352].columns = 3;
	M[353].mat = &U[1180];		/* U74 */
	M[353].rows = 3;
	M[353].columns = 3;
	M[354].mat = &U[1189];		/* U75 */
	M[354].rows = 3;
	M[354].columns = 3;
	M[355].mat = &U[1198];		/* U76 */
	M[355].rows = 3;
	M[355].columns = 3;
	M[356].mat = &U[1207];		/* U77 */
	M[356].rows = 3;
	M[356].columns = 3;
	M[357].mat = &U[1216];		/* U78 */
	M[357].rows = 3;
	M[357].columns = 3;
	M[358].mat = &U[1225];		/* U79 */
	M[358].rows = 3;
	M[358].columns = 3;
	M[359].mat = &U[1234];		/* U80 */
	M[359].rows = 3;
	M[359].columns = 3;
	M[360].mat = &U[1243];		/* U81 */
	M[360].rows = 3;
	M[360].columns = 3;
	M[361].mat = &U[1252];		/* U82 */
	M[361].rows = 3;
	M[361].columns = 3;
	M[362].mat = &U[1261];		/* U83 */
	M[362].rows = 3;
	M[362].columns = 3;
	M[363].mat = &U[1270];		/* U84 */
	M[363].rows = 3;
	M[363].columns = 3;
	M[364].mat = &U[1279];		/* U85 */
	M[364].rows = 3;
	M[364].columns = 3;
	M[365].mat = &U[1288];		/* U86 */
	M[365].rows = 3;
	M[365].columns = 3;
	M[366].mat = &U[1297];		/* U87 */
	M[366].rows = 3;
	M[366].columns = 3;
	M[367].mat = &U[1306];		/* U88 */
	M[367].rows = 3;
	M[367].columns = 3;
	M[368].mat = &U[1315];		/* U89 */
	M[368].rows = 3;
	M[368].columns = 3;
	M[369].mat = &U[1324];		/* U90 */
	M[369].rows = 3;
	M[369].columns = 3;
	M[370].mat = &U[1333];		/* U91 */
	M[370].rows = 3;
	M[370].columns = 3;
	M[371].mat = &U[1342];		/* U92 */
	M[371].rows = 3;
	M[371].columns = 3;
	M[372].mat = &U[1351];		/* U93 */
	M[372].rows = 3;
	M[372].columns = 3;
	M[373].mat = &U[1360];		/* U94 */
	M[373].rows = 3;
	M[373].columns = 3;
	M[374].mat = &U[1369];		/* U95 */
	M[374].rows = 3;
	M[374].columns = 3;
	M[375].mat = &U[1378];		/* U96 */
	M[375].rows = 6;
	M[375].columns = 6;
	M[376].mat = &U[1414];		/* U97 */
	M[376].rows = 6;
	M[376].columns = 1;
	M[377].mat = &U[1420];		/* U98 */
	M[377].rows = 8;
	M[377].columns = 6;
	M[378].mat = &U[1468];		/* U99 */
	M[378].rows = 6;
	M[378].columns = 6;
	M[379].mat = &U[1504];		/* U100 */
	M[379].rows = 6;
	M[379].columns = 6;
	M[380].mat = &U[1540];		/* U101 */
	M[380].rows = 6;
	M[380].columns = 6;
	M[381].mat = &U[1576];		/* U102 */
	M[381].rows = 6;
	M[381].columns = 6;
	M[382].mat = &U[1612];		/* U103 */
	M[382].rows = 6;
	M[382].columns = 6;
	M[383].mat = &U[1648];		/* U104 */
	M[383].rows = 6;
	M[383].columns = 6;
	M[384].mat = &U[1684];		/* U105 */
	M[384].rows = 6;
	M[384].columns = 6;
	M[385].mat = &U[1720];		/* U106 */
	M[385].rows = 6;
	M[385].columns = 6;
	M[386].mat = &U[1756];		/* U107 */
	M[386].rows = 6;
	M[386].columns = 6;
	M[387].mat = &U[1792];		/* U108 */
	M[387].rows = 6;
	M[387].columns = 6;
	M[388].mat = &U[1828];		/* U109 */
	M[388].rows = 6;
	M[388].columns = 6;
	M[389].mat = &U[1864];		/* U110 */
	M[389].rows = 6;
	M[389].columns = 6;
	M[390].mat = &U[1900];		/* U111 */
	M[390].rows = 6;
	M[390].columns = 6;
	M[391].mat = &U[1936];		/* U112 */
	M[391].rows = 6;
	M[391].columns = 6;
	M[392].mat = &U[1972];		/* U113 */
	M[392].rows = 6;
	M[392].columns = 6;
	M[393].mat = &U[2008];		/* U114 */
	M[393].rows = 6;
	M[393].columns = 6;
	M[394].mat = &U[2044];		/* U115 */
	M[394].rows = 6;
	M[394].columns = 6;
	M[395].mat = &U[2080];		/* U116 */
	M[395].rows = 6;
	M[395].columns = 6;
	M[396].mat = &U[2116];		/* U117 */
	M[396].rows = 6;
	M[396].columns = 6;
	M[397].mat = &U[2152];		/* U118 */
	M[397].rows = 6;
	M[397].columns = 6;
	M[398].mat = &U[2188];		/* U119 */
	M[398].rows = 6;
	M[398].columns = 6;
	M[399].mat = &U[2224];		/* U120 */
	M[399].rows = 6;
	M[399].columns = 6;
	M[400].mat = &U[2260];		/* U121 */
	M[400].rows = 6;
	M[400].columns = 6;
	M[401].mat = &U[2296];		/* U122 */
	M[401].rows = 6;
	M[401].columns = 6;
	M[402].mat = &U[2332];		/* U123 */
	M[402].rows = 6;
	M[402].columns = 6;
	M[403].mat = &U[2368];		/* U124 */
	M[403].rows = 6;
	M[403].columns = 6;
	M[404].mat = &U[2404];		/* U125 */
	M[404].rows = 6;
	M[404].columns = 6;
	M[405].mat = &U[2440];		/* U126 */
	M[405].rows = 6;
	M[405].columns = 6;
	M[406].mat = &U[2476];		/* U127 */
	M[406].rows = 6;
	M[406].columns = 6;
	M[407].mat = &U[2512];		/* U128 */
	M[407].rows = 6;
	M[407].columns = 6;
	M[408].mat = &U[2548];		/* U129 */
	M[408].rows = 6;
	M[408].columns = 6;
	M[409].mat = &U[2584];		/* U130 */
	M[409].rows = 6;
	M[409].columns = 6;
	M[410].mat = &U[2620];		/* U131 */
	M[410].rows = 6;
	M[410].columns = 6;
	M[411].mat = &U[2656];		/* U132 */
	M[411].rows = 6;
	M[411].columns = 6;
	M[412].mat = &U[2692];		/* U133 */
	M[412].rows = 6;
	M[412].columns = 6;
	M[413].mat = &U[2728];		/* U134 */
	M[413].rows = 6;
	M[413].columns = 6;
	M[414].mat = &U[2764];		/* U135 */
	M[414].rows = 6;
	M[414].columns = 6;
	M[415].mat = &U[2800];		/* U136 */
	M[415].rows = 6;
	M[415].columns = 6;
	M[416].mat = &U[2836];		/* U137 */
	M[416].rows = 6;
	M[416].columns = 6;
	M[417].mat = &U[2872];		/* U138 */
	M[417].rows = 6;
	M[417].columns = 6;
	M[418].mat = &U[2908];		/* U139 */
	M[418].rows = 6;
	M[418].columns = 6;
	M[419].mat = &U[2944];		/* U140 */
	M[419].rows = 6;
	M[419].columns = 6;
	M[420].mat = &U[2980];		/* U141 */
	M[420].rows = 6;
	M[420].columns = 6;
	M[421].mat = &U[3016];		/* U142 */
	M[421].rows = 6;
	M[421].columns = 6;
	M[422].mat = &U[3052];		/* U143 */
	M[422].rows = 6;
	M[422].columns = 6;
	M[423].mat = &U[3088];		/* U144 */
	M[423].rows = 6;
	M[423].columns = 6;
	M[424].mat = &U[3124];		/* U145 */
	M[424].rows = 6;
	M[424].columns = 6;
	M[425].mat = &U[3160];		/* U146 */
	M[425].rows = 6;
	M[425].columns = 6;
	M[426].mat = &U[3196];		/* U147 */
	M[426].rows = 6;
	M[426].columns = 6;
	M[427].mat = &U[3232];		/* U148 */
	M[427].rows = 6;
	M[427].columns = 6;
	M[428].mat = &U[3268];		/* U149 */
	M[428].rows = 6;
	M[428].columns = 6;
	M[429].mat = &U[3304];		/* U150 */
	M[429].rows = 6;
	M[429].columns = 6;
	M[430].mat = &U[3340];		/* U151 */
	M[430].rows = 6;
	M[430].columns = 6;
	M[431].mat = &U[3376];		/* U152 */
	M[431].rows = 6;
	M[431].columns = 6;
	M[432].mat = &U[3412];		/* U153 */
	M[432].rows = 6;
	M[432].columns = 6;
	M[433].mat = &U[3448];		/* U154 */
	M[433].rows = 6;
	M[433].columns = 6;
	M[434].mat = &U[3484];		/* U155 */
	M[434].rows = 6;
	M[434].columns = 6;
	M[435].mat = &U[3520];		/* U156 */
	M[435].rows = 6;
	M[435].columns = 6;
	M[436].mat = &U[3556];		/* U157 */
	M[436].rows = 6;
	M[436].columns = 6;
	M[437].mat = &U[3592];		/* U158 */
	M[437].rows = 6;
	M[437].columns = 6;
	M[438].mat = &U[3628];		/* U159 */
	M[438].rows = 6;
	M[438].columns = 6;
	M[439].mat = &U[3664];		/* U160 */
	M[439].rows = 6;
	M[439].columns = 6;
	M[440].mat = &U[3700];		/* U161 */
	M[440].rows = 6;
	M[440].columns = 6;
	M[441].mat = &U[3736];		/* U162 */
	M[441].rows = 6;
	M[441].columns = 6;
	M[442].mat = &U[3772];		/* U163 */
	M[442].rows = 6;
	M[442].columns = 6;
	M[443].mat = &U[3808];		/* U164 */
	M[443].rows = 6;
	M[443].columns = 6;
	M[444].mat = &U[3844];		/* U165 */
	M[444].rows = 6;
	M[444].columns = 6;
	M[445].mat = &U[3880];		/* U166 */
	M[445].rows = 6;
	M[445].columns = 6;
	M[446].mat = &U[3916];		/* U167 */
	M[446].rows = 6;
	M[446].columns = 6;
	M[447].mat = &U[3952];		/* U168 */
	M[447].rows = 6;
	M[447].columns = 6;
	M[448].mat = &U[3988];		/* U169 */
	M[448].rows = 6;
	M[448].columns = 6;
	M[449].mat = &U[4024];		/* U170 */
	M[449].rows = 6;
	M[449].columns = 6;


		/* end of initialization phase */
		initialize = false;
		return initialize;
	}
}

/* Macro to generate component library
 * Can be modified if the component is part of a big project with other components 
 */
ORO_CREATE_COMPONENT(RCCMotionStack::YouBot_control)


