...
void XXMatrixToVector(XXMatrix *mat_source, std::vector<std::vector<double> > *mat_dest)
{
	int rows,columns;
	columns=mat_source->columns;
	rows=mat_source->rows;
	mat_dest->resize(rows);
	for(int i=0;i<rows;i++)
	{
		mat_dest->at(i).resize(columns);
		for(int j=0;j<columns;j++)
			mat_dest->at(i)[j]=mat_source->mat[j+i*columns];

	}
}
void XXVectorToMatrix( std::vector<std::vector<double> > *mat_source, XXMatrix *mat_dest)
{
	//TODO check memory leaks!!!!!!!!
	int rows,columns;
	rows=mat_source->size();
	columns=mat_source->at(0).size();
	for(int i=0;i<rows;i++)
	{
		for(int j=0;j<columns;j++)
			mat_dest->mat[j+i*columns]=mat_source->at(i)[j];
